...
export function buildExpandPrompt(m: Memory): string {
  return [
    "A user saved this personal memory. Generate 3-5 short search phrases someone",
    "might type WEEKS LATER to find it again — deliberately using DIFFERENT words",
    "than the memory itself: synonyms, the problem named by its symptom or",
    "consequence, related concepts. Mix German and English naturally (the vault is",
    "bilingual). One phrase per line. No numbering, no quotes, no commentary.",
    "",
    `Title: ${m.fm.title}`,

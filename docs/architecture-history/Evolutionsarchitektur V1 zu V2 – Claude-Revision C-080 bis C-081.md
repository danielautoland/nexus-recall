# Bastra Recall – Evolutionsarchitektur V1 → V2 (Claude-Revision C-080 bis C-081)
...
> C-068–C-073, C-074–C-077 und C-078–C-079. Diese Datei ersetzt keine davon;
> alle elf bleiben nebeneinander erhalten. Die englische Schwesterfassung
> `docs/Evolution Architecture V1 to V2.md` wird weiterhin nicht nachgeführt.
>
> Neu in dieser Fassung sind ausschließlich die Deltas C-080–C-081. Beide
> ziehen eine Regel auf das zurück, was Code und Artefakt tatsächlich einlösen:
> C-080 korrigiert die Bedeutung des vorhandenen `bridge`-Feldes, C-081 macht
> den eingefrorenen Graph-Snapshot nachweispflichtig. Es handelt sich um
...
> C-001–C-079 bleibt als historische Basis erhalten und wird nicht umgedeutet;
...
>
> Nächste freie ID nach dieser Runde: C-082.
...
**Nächste freie ID: C-082.** Neue Delta-Reviews beginnen dort. Ein Urteil
...
Memory-Inhalt. Das Queue- beziehungsweise Run-Artefakt speichert mindestens:
...
## 28. Delta-Ledger (C-029–C-081)

Dieser Abschnitt dokumentiert zehn aufeinanderfolgende Runden von Deltas
...
**Runde 10 — C-080 und C-081** ist der abschließende Delta-Fix. Beide Einträge
betreffen dieselbe Runde 9 und dieselbe Schwachstelle: Eine Regel war
ausführbar formuliert, stützte sich aber auf eine Zusage, die weder der Code
noch das Artefakt einlöst:
...
- **Evidenz:** C-078 verlangte einen „eingefrorenen Graph-Snapshot“, ohne zu
  ...
  darin fehlt.
- **Gate:** keines für die Zuordnung selbst — sie bleibt read-only Sidecar-
  Arbeit im Sinne von C-018. Die persistente Gestalt des Artefakts fällt unter
  den Schemaentscheid nach M4.
...
## 32. Übergabe nach dem abschließenden Delta-Fix

**Was geändert wurde.** Diese Fassung fügt die Deltas C-080 und C-081 hinzu.
Geändert wurden ausschließlich die folgenden Zeilenbereiche:

| Passage | Zeilen | Delta |
|---|---|---|
| Titel und Präambel | 1–32 | alle |
| 0.4 Korrekturverweise und Ledgerzeilen C-078 bis C-081 | 181–184 | alle |
| 0.4 Abnahmeblock und nächste freie ID | 279–296 | alle |
| 6.3 Stufe-3-Vorrang und historieunabhängiges Kriterium | 1143–1168 | C-080 |
| 6.3 Bestandteile des Strukturkriteriums und `bridge`-Abgrenzung | 1183–1207 | C-080 |
| 6.3 Graphausfall und Snapshot-Nachweis | 1229–1280 | C-081 |
| 14.4 unbekannter Reason-Code | 2507–2516 | C-081 |
| 18.5 M4-Gates | 3392–3407 | C-080, C-081 |
| 24 Verbotsliste | 3901–3909 | C-080, C-081 |
| 28 Überschrift und Rundenzahl | 4047–4049 | alle |
| 28 Rundenlabel 5, 8 und 9 entklammert | 4113–4159 | redaktionell |
| 28 Runde-10-Zuordnungstabelle | 4166–4174 | alle |
| 28 Korrekturverweis und Abnahmekriterium an C-076 | 5529–5541 | C-080 |
| 28 Evidenz, Abnahmekriterium und Korrekturverweis an C-078 | 5585–5605 | C-080, C-081 |
| 28 Korrekturverweis an C-079 sowie Delta-Blöcke C-080 und C-081 | 5637–5719 | alle |
| 32 dieser Abschnitt | ab 5995 | — |

Alle übrigen Passagen sind unangetastet. Produktcode, englische
Schwesterfassung und die zehn Vorgängerfassungen wurden nicht verändert.

**Was die beiden Deltas beheben.** Beide schließen dieselbe Art von Lücke: Eine
Regel war ausführbar formuliert, stützte sich aber auf eine Zusage, die weder
der Code noch das Artefakt einlöst.

1. **Die Bridge-Semantik (C-080).** Das Strukturkriterium nahm die
   Brückeneigenschaft mit der Lesart auf, der Knoten verbinde „sonst getrennte
   Cluster“. `bridgeFor` prüft das nicht: Es sammelt die Cluster der Nachbarn,
   entfernt den eigenen und liefert die Restliste, sobald zwei verschiedene
   fremde Cluster übrig bleiben. Ob diese Cluster ohne den Knoten unverbunden
   wären, wird nie berechnet. Die Eigenschaft heißt jetzt überall, was sie ist —
   clusterübergreifende Nachbarschaft — und bleibt mit dieser Bedeutung
   zulässiger Bestandteil des Kriteriums. Eine echte Artikulations- oder
   Brückenanalyse ist als zusätzliche, heute nicht geleistete Grapharbeit
   ausgewiesen.
2. **Der Snapshot-Nachweis (C-081).** „Eingefroren“ war eine Zusage ohne
   Nachweis. Ein Zeitstempel sagt, wann gerechnet wurde, nicht worauf — und der
   Live-Graph bewegt sich mit jedem Save. Das Queue- beziehungsweise
   Run-Artefakt hält jetzt Projektionsschema samt Version, Snapshot-Hash,
   Erstellungszeitpunkt, das angewandte Kriterium samt Schwellenwert oder
   Quantil und je zugeordnetem historieunbekannten Memory dessen ID, `degree`,
   fremde Cluster beziehungsweise `bridge`-Wert und die resultierende Stufe
   fest; alternativ wird der vollständige Snapshot content-addressed
   persistiert und referenziert. Im laufenden Review wird nicht neu berechnet
   und nicht neu zugeordnet, ein Neustart setzt dieselbe Warteschlange fort,
   und ein Reason-Code außerhalb des geltenden Vokabulars löst keine
   Wiedervorlage aus.

**Ist-Claims.** Die beiden in C-078 zugelassenen Bestandteile existieren
bereits, aber nur mit der Bedeutung, die C-080 festhält: Die Graph-Projektion
führt je Knoten die Kantenzahl (`packages/core/src/graph.ts:36`) und setzt das
Feld `bridge` (`:38`–`:39`), sobald ein Knoten Nachbarn in mindestens zwei
unterschiedlichen fremden Clustern besitzt
(`packages/core/src/graph.ts:302`–`:305`); erzeugt wird beides von `buildGraph`
(`:226`). Eine Prüfung, ob diese Cluster ohne den Knoten unverbunden wären,
findet nicht statt. Für das Strukturkriterium ist keine neue Graphauswertung zu
bauen; für eine echte Artikulationsanalyse wäre es eine.
...
liegt zehn Revisionen zurück. Nebenbefund ohne C-ID: Die Daemon-README
beschreibt abgelaufene Memories als „(or excluded if expired)"; der Code dämpft
sie nur auf 20 %.

**Nächste freie ID: C-082.**

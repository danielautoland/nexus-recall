# Bastra Recall – Evolutionsarchitektur V1 → V2 (Claude-Revision C-063 bis C-067)
...
> C-029–C-039, C-040–C-048, C-049–C-054, C-055–C-059 und C-060–C-062. Diese
> Datei ersetzt keine davon; alle sieben bleiben nebeneinander erhalten. Die
> englische Schwesterfassung `docs/Evolution Architecture V1 to V2.md` wird
> weiterhin nicht nachgeführt.
>
> Neu in dieser Fassung sind ausschließlich die Deltas C-063–C-067. Sie setzen
> die am 25. Juli 2026 getroffenen Product-Owner-Entscheidungen um; Abschnitt
> 31 wechselt damit von einer Entscheidungsvorlage zu einem
> Entscheidungsprotokoll. Es handelt sich um einen reinen Delta-Fix, nicht um
> eine Vollrevision: Passagen ohne Delta sind unverändert. Jede geänderte
> Passage ist über das Ledger in 0.4 und das Delta-Ledger in Abschnitt 28 auf
> genau eine C-ID zurückführbar. Der Stand C-001–C-062 bleibt als historische
> Basis erhalten und wird nicht umgedeutet; wo ein neuer Eintrag einen älteren
...
>
> Nächste freie ID nach dieser Runde: C-068.
...
| C-057 | Messproblem | **Ergänzt durch C-066:** Der Cue-Erzeugungsweg wird als eigener, vorab registrierter Faktor geführt. M0 legt Power und Mindest-N je Zelle des 2×2-Cue-Versuchs versioniert fest und stellt getrennte beschreibende und assoziative Goldfälle bereit; bei zu kleinem N bleibt die Interaktion explorativ und trägt keine Live-Freigabe. |
...
| C-063 | Product-Owner-Entscheid | Der Bestätigungsbezug entsteht in der Recall-Oberfläche: `user_asserted` entsteht ausschließlich durch die ausdrückliche Nutzerbestätigung beim Review, nie automatisch im Save-Pfad. Die vier Antwortmöglichkeiten der Oberfläche bilden vier Provenienzklassen ab. |
| C-064 | Product-Owner-Vorgabe | Eine Bestätigung gilt für genau ein Memory in genau einem Inhaltsstand. Sie ist nicht übertragbar, nicht wiederverwendbar und verfällt bei inhaltlicher Änderung des bestätigten Memorys. |
| C-065 | Product-Owner-Entscheid | Der gesamte Bestand wird geprüft. `not_scheduled` bezeichnet nur noch die Warteschlangenposition und keine Ausnahme; die Prüfung läuft in vier Prioritätsstufen und endet je Memory mit geklärter oder ausdrücklich bestätigt unklarer Herkunft. |
| C-066 | Product-Owner-Entscheid | Abschnitt 31 wird zum Entscheidungsprotokoll: Cue-Erzeugung vertagt und in M2 als vorab registrierter Faktor geprüft, genau ein handlungsorientierter externer Benchmark in V1.x, Deep Recall Stufe 2 erst nach Stufe-1-Messung, Herkunft jetzt read-only prüfen und Schemafelder gemeinsam nach M4. |
| C-067 | Product-Owner-Vorgabe | Ein abgelehnter Konsolidierungsvorschlag kehrt nur zurück, wenn er inhaltlich ein anderer ist; ein geänderter Fingerprint ist notwendige, aber nicht hinreichende Bedingung. |
...
**Product-Owner-Entscheidungen, 25.07.2026 (diese Fassung):** Die vier in
...
**Nächste freie ID: C-068.** Neue Delta-Reviews beginnen dort. Ein Urteil
...
| Beobachtung | abgeleitete Klasse |
|---|---|
| `write_origin: user-directed`, mit oder ohne Auditkontext | `unknown_legacy`, Review-Status `pending` |
...
Antwortmöglichkeiten bilden unmittelbar die Provenienzklassen ab:

| Auswahl in der Oberfläche | resultierende `provenance_class` |
|---|---|
| „Ja, das stammt von mir.“ | `user_asserted` |
| „Das hat der Agent beobachtet oder abgeleitet.“ | `agent_observed` |
...
Ein nicht importierter Save mit `write_origin: user-directed` ist die einzige
Konstellation, die überhaupt für eine spätere Bestätigung in Betracht kommt —
automatisch entsteht `user_asserted` auch dort nicht. `agent-session` bedeutet
lediglich, dass der Agent geschrieben hat — nicht, dass der Inhalt eine Agentenbeobachtung ist; eine vom
...
- **Eindeutig gebunden.** Er verweist auf die Identität des bestätigten Memorys
  und auf dessen Inhaltsstand zum Zeitpunkt der Bestätigung.
- **Nicht wiederverwendbar.** Er ist für genau diese eine Entscheidung gültig
  ...
  angewandt werden.
- **Verfallend bei Inhaltsänderung.** Ändert sich der bestätigte Inhalt, verliert
  die Bestätigung ihre Wirkung; das Memory fällt auf die abgeleitete Klasse
  zurück und erhält den Review-Status `pending`. Eine Umformulierung darf keine
  fremde Aussage unter eine alte Zustimmung schieben.
...
erscheinen. Ein Vorschlag, der sich nur in einer Randgröße unterscheidet,
ansonsten aber dieselbe abgelehnte Operation an derselben Struktur ist, gilt
nicht als neuer Vorschlag. Im Zweifel unterbleibt die Wiedervorlage: Ein
verpasster Verbesserungsvorschlag kostet weniger als ein Vorschlag, der jede
Woche in leicht anderer Form zurückkehrt. Andernfalls entstünde genau die Schleife, die diese Regel verhindert
...
- für den Cue-Erzeugungsweg aus 18.3 werden Zellstruktur, Mindest-N und
  Auswertungsregel ebenfalls vor dem Lauf festgelegt und versioniert abgelegt —
  getrennt vom 2×2 der Cue-Achsen.
...
**Erzeugungsweg als eigener Faktor.** Der Product-Owner-Entscheid aus 31
vertagt die Wahl zwischen agentengenerierten und batch-generierten Cues und
gibt sie stattdessen in M2 zur kontrollierten Prüfung. Das ist kein zusätzlicher
Arm im obigen 2×2: Ein ungeplantes Kreuzen verdoppelt die Zellen und zerstört
nach C-057 die Auswertbarkeit der Interaktion. Der Erzeugungsweg wird deshalb
als **eigener, vorab registrierter Faktor** geführt — mit eigener Zellstruktur,
eigenem Mindest-N und einer vor dem Lauf festgelegten Auswertungsregel. Ob er
gemeinsam mit den Cue-Achsen oder in einem getrennten Lauf geprüft wird, ist
Teil dieser Registrierung und wird nicht während des Laufs entschieden.
...
- ein abgelehnter Vorschlag kehrt nur zurück, wenn er inhaltlich ein anderer
  ist; ein geänderter Fingerprint allein löst keine Wiedervorlage aus.
  Ablehnungsprotokoll und Vorschlagsvergleich sind je Struktur nachweisbar;
...
- kein Klasse-B-Operator erzeugt eine Version, und keine dauerhafte
  ...
  Pin-Override;
- kein Memory erhält automatisch `user_asserted` ohne serverseitig
  beziehungsweise UI-adapterseitig erzeugten, vom speichernden Caller nicht
  behauptbaren und später auflösbaren Bestätigungsbezug;
...
- jede Bestätigung ist an genau ein Memory und dessen Inhaltsstand gebunden und
  verfällt bei inhaltlicher Änderung;
...
- der Bestand vollständig geprüft ist und jedes Memory eine geklärte oder
  bestätigt unklare Herkunft trägt;
...
## 28. Delta-Ledger (C-029–C-067)

Dieser Abschnitt dokumentiert sechs aufeinanderfolgende Runden von Deltas
...
- **Rollback:** Additive Sidecar-Felder. Ohne sie verhält sich das System wie
  ...
  Verweigerung einer unbelegten Einstufung.
- **Ergänzt und eingeschränkt durch C-055 und C-060.** Dieser Eintrag sichert
  die Importherkunft ab, behandelt ein nicht importiertes
  `write_origin: user-directed` aber weiterhin als hinreichend. Verbindlich ist
  die Fassung in 6.3: Ohne einen serverseitig oder UI-adapterseitig erzeugten,
  nicht selbst behauptbaren Bestätigungsbezug entsteht kein `user_asserted`;
  Attestierung ist kein Attribut eines Schreibpfads, und kein heutiger Pfad
  erfüllt sie. Der Review-Status heißt jetzt `not_scheduled` statt
...
- **Beantwortet durch C-063, verschärft durch C-064, korrigiert durch C-065.**
  Der Bestätigungsbezug entsteht in der Recall-Oberfläche; eine Bestätigung
  gilt für genau ein Memory in genau einem Inhaltsstand; und `not_scheduled`
  ist keine Ausnahme von der Prüfung mehr, sondern nur noch die
  Warteschlangenposition.
...
- **Datenquelle:** der Vault-Bestand selbst; Nutzungshäufigkeit aus der
  Telemetrie für die Priorisierung der zweiten Stufe.
...
**Auflage:** Der Erzeugungsweg ist nach 18.3 ein eigener, **vorab registrierter
Faktor** mit eigener Zellstruktur, eigenem Mindest-N und vor dem Lauf
festgelegter Auswertungsregel. Er wird nicht ungeplant mit dem 2×2 der
Cue-Achsen gekreuzt — das würde nach C-057 die Interaktion unauswertbar machen.
...
**Auswirkung:** Die vier Antwortmöglichkeiten der Oberfläche bilden vier
Provenienzklassen ab (6.3). `not_scheduled` bezeichnet nur noch die
...
## 32. Übergabe nach den Product-Owner-Entscheidungen

**Was geändert wurde.** Diese Fassung setzt die am 25. Juli 2026 getroffenen
Product-Owner-Entscheidungen als Deltas C-063–C-067 um. Abschnitt 31 wechselt
dabei von einer Entscheidungsvorlage zu einem Entscheidungsprotokoll. Geändert
wurden ausschließlich die folgenden Zeilenbereiche:

| Passage | Zeilen | Delta |
|---|---|---|
| Titel und Präambel | 1, 11–32 | alle |
| 0.4 Korrekturverweise an C-042, C-049, C-055, C-057, C-060, C-062 | 144, 151, 157, 159, 162, 164 | C-063–C-067 |
| 0.4 Ledgerzeilen C-063–C-067 | 165–169 | alle |
| 0.4 Abnahmeblock und nächste freie ID | 232–245 | alle |
| 6.3 Klassentabelle `user_asserted` | 808 | C-063 |
| 6.3 Stufe-2-Tabelle ohne `user_asserted` | 856–864 | C-063 |
| 6.3 Rückfallsatz und Mac-Bridge-Abgrenzung | 904–920 | C-063 |
| 6.3 Wo der Bestätigungsbezug entsteht | 922–957 | C-063 |
| 6.3 Ableitungssatz für nicht importierte Saves | 967–973 | C-063 |
| 6.3 `not_scheduled` und Bestandsprüfung | 998–1030 | C-065 |
| 6.3 Bindung der Bestätigung | 1037–1054 | C-064 |
| 6.3 Provenienz-Override mit Verfall | 1064–1077 | C-063, C-064 |
| 14.4 Rückkehrregel | 2178–2185 | C-067 |
| 18.1 Registrierung des Erzeugungswegs | 2529–2531 | C-066 |
| 18.3 Erzeugungsweg als registrierter Faktor | 2705–2713 | C-066 |
| 18.5 M4-Gates | 2912–2945 | C-063–C-067 |
| 19.1 entschiedener Benchmarktyp | 3148–3154 | C-066 |
| 22 Rückwärtskompatibilität | 3323–3332 | C-063, C-065 |
| 24 Verbotsliste | 3412–3417 | C-063–C-067 |
| 25 Umsetzungsreihenfolge Punkt 6 | 3435–3436 | C-065 |
| 26.2 Promotionskriterien | 3515–3518 | C-063, C-065 |
| 28 Runde-6-Zuordnungstabelle | 3628–3637 | alle |
| 28 Korrekturverweise an C-042, C-055, C-060, C-062 | 3951, 4356, 4525, 4593 | C-063–C-067 |
| 28 Delta-Blöcke C-063–C-067 | 4602–4714 | alle |
| 31 vollständig neu als Entscheidungsprotokoll | 4888–4981 | C-066 |
| 32 dieser Abschnitt | ab 4983 | — |

Alle übrigen Passagen sind unangetastet. Produktcode, englische
Schwesterfassung und die sechs Vorgängerfassungen wurden nicht verändert.

**Was die Entscheidungen am Dokument bewegen.** Zwei der fünf haben Folgen weit
über Abschnitt 31 hinaus:

1. **Der Bestätigungsbezug hat eine Quelle (C-063).** C-060 hatte festgestellt,
   dass kein Schreibpfad einen erzeugt, und die Frage offen gelassen. Sie ist
   beantwortet: Er entsteht beim Review in der Recall-Oberfläche. Damit ist die
   Rückfallregel auf `unknown_legacy` kein Provisorium mehr, sondern der
   Normalfall des Schreibpfads — auch ein künftiges einheitliches Mutation-Audit
   ändert daran nichts. Die vier Antwortmöglichkeiten der Oberfläche bilden vier
   Provenienzklassen ab.
2. **`not_scheduled` verliert seine Ausnahmewirkung (C-065).** Die bisherige
   Fassung nahm `agent-session` und fehlendes `write_origin` von der Prüfung
   aus — also den größten Teil des Bestands. Mit dem Entscheid zur vollständigen
   Bestandsprüfung bezeichnet der Wert nur noch die Warteschlangenposition. Neu
   ist außerdem die Unterscheidung zwischen einem abgeleiteten und einem
   bestätigten `unknown_legacy`: „noch niemand hat hingesehen“ gegenüber „jemand
   hat hingesehen und konnte es nicht klären“. Nur das zweite ist ein Ergebnis.

**Was aus Entscheidungen zu Anforderungen wurde.** Zwei zuvor als
Product-Owner-Fragen geführte Punkte sind keine Wahl, sondern Anforderung: die
Bindung einer Bestätigung an genau ein Memory und einen Inhaltsstand (C-064) und
die Rückkehrregel für abgelehnte Vorschläge (C-067). Beide stehen jetzt im
Fachtext, nicht in Abschnitt 31.

**Ein Konflikt, der aufgelöst werden musste.** Entscheidung 1 vertagt die Wahl
des Cue-Erzeugungswegs und gibt ihn in M2 zur Prüfung. Das kollidiert mit C-057:
Ein zusätzlicher Faktor, ungeplant über das 2×2 der Cue-Achsen gelegt, verdoppelt
die Zellen und macht die Interaktion unauswertbar. Aufgelöst in 18.3 — der
Erzeugungsweg wird als eigener, **vorab registrierter** Faktor mit eigener
Zellstruktur und eigenem Mindest-N geführt, und ob er gemeinsam oder in einem
getrennten Lauf geprüft wird, ist Teil der Registrierung.

**Keine neuen Ist-Claims.** Über den Code entstehen in dieser Runde keine neuen
Behauptungen; die Ist-Claims der Vorrunden bleiben unverändert gültig. Die
Entscheidungen betreffen künftiges Produktverhalten, nicht den Bestand.

**Was besonders zu prüfen ist.**

1. Ob die vier Antwortmöglichkeiten in 6.3 den Raum vollständig abdecken —
   insbesondere, ob ein Memory denkbar ist, das weder Nutzeraussage noch
   Agentenbeobachtung noch Regel ist und für das „unklar“ zu grob wäre.
2. Ob das Verfallen einer Bestätigung bei Inhaltsänderung (C-064) mit dem
   Overwrite-Verhalten des Save-Pfads verträglich ist, der bestehende Felder
   bewusst erhält.
3. Ob die vier Prüfstufen in 6.3 vollständig sind und ob Stufe 2 ohne die noch
   nicht existierende Nutzungstelemetrie je Memory überhaupt priorisierbar ist.
4. Ob der Erzeugungsweg-Faktor in 18.3 tatsächlich getrennt registrierbar ist
   oder ob er faktisch doch mit den Cue-Achsen gekreuzt werden muss, weil
   dieselben Cues erzeugt werden.
5. Ob „handlungsorientiert“ in 19.1 als Auswahlkriterium scharf genug ist, um
   den konkreten Benchmark daraus abzuleiten.
6. Ob die Rückkehrregel in 14.4 (C-067) operationalisierbar ist, ohne dass
   „inhaltlich ein anderer“ selbst zu einer unentscheidbaren Größe wird.
...
Schwesterfassung liegt sechs Revisionen zurück. Nebenbefund ohne C-ID: Die
Daemon-README beschreibt abgelaufene Memories als „(or excluded if expired)“;
der Code dämpft sie nur auf 20 %.

**Was die Änderung trägt.** Für die Entscheidungen das Protokoll in Abschnitt 31;
für ihre Umsetzung die betroffenen Fachpassagen und Gates; für die
Konfliktauflösung zwischen Entscheidung 1 und C-057 der Versuchsplan in 18.3.

**Nächste freie ID: C-068.**

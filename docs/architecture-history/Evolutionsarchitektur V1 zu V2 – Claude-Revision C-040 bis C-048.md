# Bastra Recall – Evolutionsarchitektur V1 → V2 (Claude-Revision C-040 bis C-048)
...
> `docs/Evolutionsarchitektur V1 zu V2 – Claude-Revision ab C-029.md` mit den
> Deltas C-029–C-039. Diese Datei ersetzt weder das Original noch die
> Vorgängerrevision; alle drei bleiben nebeneinander erhalten.
>
> Neu in dieser Fassung sind ausschließlich die Deltas C-040–C-048 aus dem
> Codex-Gegenreview der Vorgängerrevision, zuzüglich dreier redaktioneller
> Korrekturen ohne eigene C-ID. Jede geänderte Passage ist über das Ledger in
> 0.4 und das Delta-Ledger in Abschnitt 28 auf genau eine C-ID zurückführbar.
> Der Stand C-001–C-039 bleibt als historische Basis erhalten und wird nicht
> umgedeutet; C-040–C-048 präzisieren mehrere Deltas der Vorgängerrevision,
> ohne deren Urteil aufzuheben.
>
> Nächste freie ID nach dieser Runde: C-049.
...
| C-042 | Ist-Korrektur | Ein fehlendes `provenance_class` gilt niemals als `user_asserted`. Altbestand ohne eindeutige Zuordnung wird `unknown_legacy`; nur explizites `write_origin: user-directed` wird automatisch `user_asserted`. |
...
**Codex-Gegenreview der Revision, 25.07.2026 (diese Fassung):** Der
...
**Nächste freie ID: C-049.** Neue Delta-Reviews beginnen dort. Ein Urteil
...
Vergangenheit nicht mehr rekonstruieren ließen. Ein Widerspruch setzt deshalb
`retracted_at`, er löscht nichts.
...
und widersprochene Fakten zeitlich invalidiert statt sie zu löschen.
...
Verbindliches Mapping beim Ableiten der `provenance_class`:

| Beobachtung am Bestandsmemory | abgeleitete Klasse |
|---|---|
| `write_origin: user-directed` | `user_asserted` |
| `write_origin: capture-review` | `unknown_legacy`, zur Nutzerbestätigung vorgemerkt |
| `write_origin: agent-session` | `unknown_legacy` |
| `write_origin` fehlt | `unknown_legacy` |
| `source` mit Import-Adapter-Präfix | `unknown_legacy`, zusätzlich als importiert markiert |

Nur der erste Fall wird automatisch zu `user_asserted`. `agent-session`
bedeutet lediglich, dass der Agent geschrieben hat — nicht, dass der Inhalt
eine Agentenbeobachtung ist; eine vom Nutzer diktierte Regel und eine
selbstgezogene Schlussfolgerung sind darin ununterscheidbar. Deshalb wird
`agent-session` nicht nach `agent_observed` gemappt, sondern nach
`unknown_legacy`.

`confidence` wird für dieses Mapping ausdrücklich **nicht** herangezogen. Das
Feld hat heute einen Schema-Default von 1,0, wird beim Retrieval nirgends
gelesen und trägt deshalb keine Information, aus der sich Herkunft ableiten
ließe.

`unknown_legacy` ist keine Vertrauensklasse, sondern das Eingeständnis, dass
die Herkunft unbekannt ist. Es schützt nicht wie `user_asserted` und
qualifiziert nicht zum Überschreiben wie `derived`. Ein `unknown_legacy`-Memory
wird nur durch eine ausdrückliche Nutzerbestätigung oder durch einen neuen Save
mit expliziter Klasse aus diesem Zustand herausgeführt. Ein Massen-Rewrite des
Vaults findet nicht statt.
...
|---|---|
| maximale Laufzeit | Abbruch |
| maximales Tokenbudget | Abbruch |
| maximale Zahl offener Äste | keine neuen Äste mehr |
| maximale Tiefe je Ast | Ast wird geschlossen |
| maximale Zahl Provider-Aufrufe | kein semantischer Arm mehr |
| maximale Zahl betrachteter Kandidaten | kein Poolausbau mehr |
...
**Abbruch.** Stufe 2 endet, sobald eine der folgenden Bedingungen eintritt:

1. zwei aufeinanderfolgende Schritte ohne Evidenzgewinn im obigen Sinn;
2. eine der Budgetgrenzen ist erreicht;
3. die Teilfragenliste ist abgearbeitet und kein Ast ist mehr offen.
...
| `no_answer` | die Suche war hinreichend ausgeschöpft — Bedingung 1 oder 3 — und es existiert nachweislich keine belastbare Evidenz |
| `inconclusive_budget_exhausted` | die Suche endete an einer Budgetgrenze; über das Vorhandensein von Evidenz ist damit nichts ausgesagt |
...
werden, entscheidet allein die Ablation in 18.3; bis dahin bleiben alle vier im
Prüfumfang.
...
deshalb explizit, statt sich auf die Deckelung zu verlassen. Voraussetzung
dafür ist, dass die Hop-Herkunft dem Entscheidungspunkt überhaupt zur Verfügung
steht; die Kennzeichnung wird dafür bis zum Evidenzentscheid durchgereicht.
...
typisierte Links erreichbar. Der Wert ist versioniert abgelegt und beginnt bei
zwei; er wird nur mit dokumentierter Begründung geändert. Gemessen wird die
Garantie über zwei Größen:
...
Die Cue-Schicht wird read-only beziehungsweise im Shadow über vier eigene Arme
verglichen:

1. keine abgeleiteten Cues — Referenzarm;
2. nur beschreibende Entity- und Scene-Cues;
3. nur assoziative Bridge- und Horizon-Cues;
4. alle vier Familien gemeinsam.
...
nicht gefahren. Die Frage „hilft der Cue?“ beantwortet der Vergleich der
Rankingwirkung zwischen Arm 1 und den Armen 2 bis 4.
...
- Bestandsmemories ohne `provenance_class` gelten als `unknown_legacy` und
  werden nicht massenhaft umgeschrieben.
...
## 28. Delta-Ledger (C-029–C-048)

Dieser Abschnitt dokumentiert zwei aufeinanderfolgende Runden von Deltas
...
| AgentRunbook-C 72,5 % / -R 57,8 % mit Latenzen | Tabelle 2 | durchgängig Qwen3.5-9B; Controller Qwen3.5-9B bzw. GPT-5.4-mini | deterministische Evaluatoren plus GPT-5.2 für Gotchas und Abstention | **nicht angegeben** | 200k Tokens Truncation |
...
## 32. Übergabe an den finalen Codex-Gegenreview

**Was geändert wurde.** Diese Fassung fügt der Vorgängerrevision die Deltas
C-040–C-048 hinzu. Neu geschrieben sind: Abschnitt 29 vollständig
(Quellenmatrix mit den Unterabschnitten 29.1–29.5), die Unterabschnitte zur
Hop-Baseline und zur `required`-Regel in 13.1, der Fallback-Block in 6.3, die
Budget-, Evidenzgewinn- und Ergebnisblöcke in 8.5, die Klassen A und B samt
Erreichbarkeitsgarantie in 14.4, die Gliederung von 18.6 in 18.6.1 und 18.6.2
sowie die Delta-Einträge C-040–C-048 in Abschnitt 28. Überarbeitet wurden 2.3,
6.3 Zeitachsen, 11.4, 17.5, 18.2, 18.3, 18.4, 18.5, 18.7, 22, 24, 26.2 und 31.
Der Stand C-001–C-039 bleibt als historische Basis erhalten; kein Eintrag der
Runde 1 wurde gestrichen oder umgedeutet.

**Was gegenüber der Vorgängerrevision sachlich falsch war.** Drei Punkte, alle
jetzt korrigiert:

1. Der Provenienz-Fallback `user_asserted` für Bestandsmemories ohne Feld
   (C-042). Er hätte dem gesamten Altbestand die höchste Vertrauensklasse
   zugesprochen. Der Code belegt das Gegenteil: fehlendes `write_origin`
   bedeutet heute faktisch ungeschützt.
2. Die Lesart dreier BEAM-Durchschnittswerte als Prozentzahlen (C-040, 29.5).
   0,163 / 0,325 / 0,400 stehen auf einer 0-bis-1-Skala; die Pass Rates lauten
   20 % / 25 % / 40 %.
3. Die Aussage, Ori dokumentiere kein Konvergenzkriterium der rekursiven
   Exploration (C-040, 29.5). Es ist dokumentiert, nur nicht in README oder
   Spezifikation.

**Welche neuen Claims entstanden.** Über Bastras Ist-Stand sechs neue, alle am
HEAD belegt und in Abschnitt 28 mit Datei und Zeile geführt: `valid_until` wird
an genau einer Stelle gelesen und wirkt ausschließlich als Score-Multiplikator
ohne Ausschluss; es gibt heute kein Feld mit Ereigniszeit- und keines mit
Weltgültigkeitssemantik; `write_origin` ist optional ohne Schema-Default und
endet in der Schreibkaskade auf `agent-session`; `confidence` besitzt einen
Default von 1,0 und wird im Retrieval nie gelesen; der Hook-Endpunkt setzt
`expand_hops` von sich aus auf 1 und traversiert dabei ausschließlich
`related_via`; die Hop-Kennzeichnung wird vor der Auslieferung an den Hook aus
der Antwort projiziert, weshalb die `required`-Schwelle hop-blind entscheidet.
Über Fremdsysteme entstanden keine neuen Behauptungen — die bestehenden wurden
mit Version, Fundstelle und Messkonfiguration unterlegt.

**Nebenbefund ohne eigene C-ID.** Die Daemon-README beschreibt abgelaufene
Memories als „(or excluded if expired)“. Der Code schließt sie nicht aus,
sondern dämpft sie auf 20 %. Das betrifft nicht dieses Dokument, sollte aber
in der README korrigiert werden.

**Was besonders zu prüfen ist.**

1. Ob das Paar `recorded_at`/`retracted_at` in 6.3 mit dem bestehenden
   Soft-Delete- und `obsolete`-Mechanismus kollidiert — beide beschreiben eine
   Form von Rücknahme, aber auf verschiedenen Ebenen.
2. Ob das Mapping in 6.3 den Fall `capture-review` richtig behandelt. Die
   Einstufung als `unknown_legacy` mit Bestätigungsvormerkung ist eine
   Entscheidung unter Unsicherheit, keine belegte Ableitung.
3. Ob die vier Cue-Arme in 18.3 gegeneinander trennscharf messbar sind oder ob
   Arm 4 die Arme 2 und 3 statistisch verdeckt.
4. Ob die Budgetgrenzen in 8.5 vollständig sind und ob
   `inconclusive_budget_exhausted` an allen Konsumenten — Hook, MCP, REST,
   Mindspace — unterscheidbar ankommt.
5. Ob die Durchreichung der Hop-Kennzeichnung bis zum Evidenzentscheid (13.1)
   die Lean-Projektion der Hook-Antwort verletzt, die heute bewusst nur sechs
   Felder überträgt.
6. Ob 18.6.1 ohne M4 genug messbare Kategorien behält, um überhaupt
   aussagekräftig zu sein — es bleiben vier von sieben.
7. Ob `max_provenance_hops` mit Startwert zwei für die Konsolidierungstiefe
   realistisch ist oder ob bereits `SPLIT` gefolgt von `MERGE` diesen Wert
   überschreitet.
8. Ob die Trennung von Klasse A und Klasse B in 14.4 vollständig ist —
   insbesondere, ob `SUPERSEDE` nicht faktisch auch eine
   Accessibility-Wirkung hat.

**Was die Änderung trägt.** Für die Fremdsystem-Aussagen die Primärquellen mit
Version, Fundstelle und Abrufdatum aus 29.2 bis 29.4 sowie die Lückenliste in
29.3; für die Bastra-Aussagen der Code am HEAD mit den in Abschnitt 28
genannten Fundstellen; für die Gate-Änderungen die bestehenden Messgates M0 bis
M6, die erweitert und in 18.6 gegliedert, aber in ihrer Freigabelogik nicht
verändert wurden.

**Offen für Product Owner und Codex gemeinsam.** Die vier Entscheidungen in
Abschnitt 31 sind bewusst nicht festgeschrieben. Keine Passage dieses Dokuments
setzt eine ihrer Optionen voraus.

**Nächste freie ID: C-049.**

# Bastra Recall – Evolutionsarchitektur V1 → V2 (Claude-Revision C-074 bis C-077)
...
> C-029–C-039, C-040–C-048, C-049–C-054, C-055–C-059, C-060–C-062, C-063–C-067
> und C-068–C-073. Diese Datei ersetzt keine davon; alle neun bleiben
> nebeneinander erhalten. Die englische Schwesterfassung
> `docs/Evolution Architecture V1 to V2.md` wird weiterhin nicht nachgeführt.
>
> Neu in dieser Fassung sind ausschließlich die Deltas C-074–C-077. Sie
> korrigieren vier Regeln, die in der Vorrunde entweder rechnerisch falsch,
> in sich zirkulär, empirisch unhaltbar oder zu schwach formuliert waren. Es handelt sich um
...
> C-001–C-073 bleibt als historische Basis erhalten und wird nicht umgedeutet;
...
>
> Nächste freie ID nach dieser Runde: C-078.
...
| C-073 | Architekturentscheidung | **Widerspruchsfrei gemacht durch C-075:** Der semantische Vorschlagshash umfasst auch Evidenz-, Begründungs- und Schutzkonfliktstand. Die Wiedervorlage eines abgelehnten Vorschlags verlangt geänderten strukturellen Fingerprint, geänderten semantischen Vorschlagshash **und** eine benannte materielle Änderung; gleicher Vorschlagsinhalt bleibt unterdrückt. |
...
| C-075 | Architekturentscheidung | Der semantische Vorschlagshash umfasst Operatortyp, normalisierte Zieländerung, Quell-IDs und -Versionen, betroffene Kanten, Evidenz- und Begründungsstand sowie die Schutzkonfliktauflösung; die Wiedervorlage verlangt alle drei Bedingungen. |
| C-076 | Ist-Korrektur | Fehlende Sidecar-Historie macht ein Memory nicht automatisch zu Stufe 2. Stufe 2 umfasst belegte hohe Nutzung und historieunbekannte Memories mit nachweislich hoher Strukturwirkung; Floors sind bereits Stufe 1, und eine unabhängige Pin-Quelle existiert am HEAD nicht. |
...
**Nächste freie ID: C-078.** Neue Delta-Reviews beginnen dort. Ein Urteil
...
| 2 | Memories mit **belegter** hoher Nutzung sowie historieunbekannte Memories mit **nachweislich hoher Strukturwirkung** | Ihre Herkunft wirkt sich am häufigsten aus — entweder über die Nutzung oder über die Stellung im Graphen |
...
Die vier Stufen sind vollständig und überschneidungsfrei: Jedes Memory fällt in
genau eine, und die Prüfung in der zuerst zutreffenden Stufe schließt eine
zweite Zuordnung aus.
...
Für historieunbekannte Memories entscheidet deshalb ein
**historieunabhängiges Kriterium über die Stufenzugehörigkeit**: die
**Strukturwirkung** im Graphen, also ein hoher Verlinkungsgrad oder eine
Brückenstellung zwischen sonst unverbundenen Bereichen. Wer sie nachweislich
besitzt, fällt in Stufe 2; wer sie nicht besitzt, in Stufe 4.
...
vorgeschlagene Zustandsänderung beschränken: Gelten Evidenz, Begründung und
gelöster Schutzkonflikt als materielle Änderung, müssen sie auch im Hash
liegen — sonst könnte eine materielle Änderung vorliegen, während der Hash
unverändert bleibt, und die Bedingungen widersprächen einander.

In den semantischen Vorschlagshash gehören deshalb mindestens:

- der **Operatortyp**;
- die **normalisierte Zieländerung** — was nach der Operation gelten soll;
- die **Quell-IDs und ihre Versionen**;
- die **betroffenen Kanten**, angelegt wie entfernt;
- der **Evidenz- und Begründungsstand**, auf den sich der Vorschlag stützt;
- die **Schutzkonfliktauflösung** — auch ihr Fehlen ist ein Zustand und geht in
  den Hash ein, damit ein später gelöster Konflikt ihn nachweislich bewegt.
...
Zeit-, Cache-, Telemetrie- oder Projektionsänderungen bewegen weder den
Fingerprint noch den Hash und bleiben damit auf beiden Wegen ausgeschlossen. Lässt sich
...
- ein abgelehnter Vorschlag kehrt nur zurück, wenn struktureller Fingerprint
  **und** semantischer Vorschlagshash sich geändert haben und mindestens eine
  ...
  Kanten, Evidenz- und Begründungsstand sowie die Schutzkonfliktauflösung;
  Ablehnungsprotokoll, Hash und Vorschlagsvergleich sind je Struktur
  nachweisbar;
...
## 28. Delta-Ledger (C-029–C-077)

Dieser Abschnitt dokumentiert acht aufeinanderfolgende Runden von Deltas
...
**Runde 8 — C-074 bis C-077** ist die abschließende Korrekturrunde. Die
...
- **Rollback:** Die Prüfung ändert keine Memory-Inhalte und lässt sich jederzeit
  ...
  nutzbar.
- **Präzisiert durch C-071, korrigiert durch C-076.** Die Datenquelle für
  Prüfstufe 2 ist der vorhandene Usage-Sidecar; es ist keine neue Telemetrie zu
  bauen, und fehlende Historie bedeutet `unknown`, nicht null. Sie macht ein
  Memory aber nicht automatisch zu Stufe 2 — dort entscheidet dann die
  Strukturwirkung.
...
- **Rollback:** Fällt die Quelle aus, entscheidet für alle Memories das
  historieunabhängige Kriterium aus 6.3 — die Strukturwirkung im Graphen; die
  Prüfung bleibt vollständig, nur ihre Priorisierung wird gröber.
...
- **Rollback:** Konservativer Fallback ist das Unterbleiben der Wiedervorlage.
- **Widerspruchsfrei gemacht durch C-075.** Der hier genannte „Hash des
  normalisierten Vorschlagsinhalts“ umfasste nur die Zustandsänderung, während
  Evidenz, Begründung und Schutzkonfliktauflösung als materielle Änderung
  galten — eine materielle Änderung hätte den Hash also unberührt lassen können.
  Verbindlich ist der Umfang aus 14.4.
...
- **Abnahmekriterium:** Der semantische Vorschlagshash umfasst mindestens
  Operatortyp, normalisierte Zieländerung, Quell-IDs und -Versionen, betroffene
  Kanten, Evidenz- und Begründungsstand sowie die Schutzkonfliktauflösung. Eine
...
- **Abnahmekriterium:** Stufe 2 umfasst Memories mit belegter hoher Nutzung
  ...
  hoher Verlinkungsgrad oder Brückenstellung. Importierte Memories bleiben
...
## 32. Übergabe nach der abschließenden Korrekturrunde

**Was geändert wurde.** Diese Fassung behebt vier Befunde als Deltas
C-074–C-077. Geändert wurden ausschließlich die folgenden Zeilenbereiche:

| Passage | Zeilen | Delta |
|---|---|---|
| Titel und Präambel | 1–33 | alle |
| 0.4 Korrekturverweise an C-065, C-067, C-069, C-071, C-072, C-073 | 167–175 | C-074–C-077 |
| 0.4 Ledgerzeilen C-074–C-077 | 176–179 | alle |
| 0.4 Datierung des Vorrunden-Blocks | 253 | — |
| 0.4 Abnahmeblock und nächste freie ID | 263–276 | alle |
| 6.3 progressive Rückfrage ohne Vorbelegung | 988–990 | C-077 |
| 6.3 Systemvorschlag ohne Vorauswahl | 997–1016 | C-077 |
| 6.3 Stufentabelle als Mitgliedschaftskriterien | 1101–1116 | C-076 |
| 6.3 Zuordnung historieunbekannter Memories | 1130–1150 | C-076 |
| 14.4 semantischer Vorschlagshash | 2326–2360 | C-075 |
| 18.1 Registrierung der Versuchsanlage | 2708–2716 | C-074 |
| 18.1 M0-Gate | 2726–2729 | C-074 |
| 18.3 Anlagen A und B | 2899–2929 | C-074 |
| 18.3 statistische Ausführbarkeit je Anlage | 2943–2955 | C-074 |
| 18.3 konstante Umgebung mit Geltungsbereich | 2957–2968 | C-074 |
| 18.5 M4-Gates | 3144–3189 | C-075, C-077 |
| 24 Verbotsliste | 3667–3677 | C-074, C-076, C-077 |
| 28 Überschrift und Rundenzahl | 3815–3817 | alle |
| 28 Runde-8-Zuordnungstabelle | 3915–3923 | alle |
| 28 Korrekturverweis an C-057 | 4715–4719 | C-074 |
| 28 Korrekturverweis an C-065 | 4968–4972 | C-076 |
| 28 Korrekturverweise an C-068, C-069, C-071, C-072, C-073 | 5053–5182 | C-074–C-077 |
| 28 Delta-Blöcke C-074–C-077 | 5190–5297 | alle |
| 31 Entscheidung 1 Auflage | 5488–5493 | C-074 |
| 31 Entscheidung 5 Auswirkung | 5556–5560 | C-077 |
| 32 dieser Abschnitt | ab 5573 | — |

Alle übrigen Passagen sind unangetastet. Produktcode, englische
Schwesterfassung und die acht Vorgängerfassungen wurden nicht verändert.

**Was falsch war.**

1. **Anlage A hatte vier Zellen (C-074).** Sind die Cue-Achsen festgehalten,
   existiert das 2×2 nicht mehr — es bleiben zwei Bedingungen. Zusätzlich
   fehlte die Trennung zwischen der Auswahl der festzuhaltenden Konfiguration
   und dem Vergleich; ohne sie ist der Vergleich durch die Auswahl kontaminiert.
   Und nur ein gekreuztes Design kann Interaktionen messen.
2. **Der Wiedervorlagevertrag war zirkulär (C-075).** Der Hash umfasste nur die
   Zustandsänderung, während Begründung auf neuer Evidenz und gelöster
   Schutzkonflikt als materielle Änderung galten. Für zwei der fünf Größen
   schlossen sich Bedingung 2 und 3 damit gegenseitig aus.
3. **`unknown` führte pauschal nach Stufe 2 (C-076).** Fehlende Historie ist
   keine Aussage über Nutzung und rechtfertigt keine Höherstufung. Das
   angebotene Zweitkriterium war zudem unbrauchbar: Floors sind bereits Stufe 1,
   und `pinned` wird im Curator-Eingang hart auf `false` gesetzt — im
   Session-Hook ist „pinned" nur der Anzeigename des Floor-Blocks.
4. **Die Antwort durfte vorbelegt sein (C-077).** Ein Default, den man
   wegklickt, belegt keine Urheberschaft — und hebt damit genau die Eigenschaft
   auf, um derentwillen die Attestierung an einen Nutzerakt gebunden wurde.

**Neue Ist-Claims, am HEAD belegt.** `packages/daemon/src/curator-run.ts:108`
setzt `pinned: false`; `packages/daemon/src/session-hook.ts:36` und `:174`
zeigen, dass „pinned" dort der Anzeigename der über `fetchFloors` geholten
Floors ist. Eine vom Floor unabhängige Pin-Quelle existiert nicht.

**Was besonders zu prüfen ist.**

1. Ob die Auswahl-/Holdout-Trennung in 18.3 mit der Goldset-Größe verträglich
   ist — zwei Teilmengen halbieren die je Bedingung verfügbaren Fälle.
2. Ob „nachweislich hohe Strukturwirkung" in 6.3 einen Schwellenwert braucht
   oder als relatives Kriterium innerhalb des Bestands ausreicht.
3. Ob der Evidenz- und Begründungsstand im semantischen Hash normalisierbar
   ist, ohne dass jede Umformulierung ihn bewegt.
4. Ob die Unwirksamkeit eines Weiter-Klicks ohne Auswahl an allen Oberflächen
   durchsetzbar ist oder ob eine Oberfläche einen impliziten Default braucht.

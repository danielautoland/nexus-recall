# Bastra Recall – Evolutionsarchitektur V1 → V2 (Claude-Revision C-078 bis C-079)
...
> C-068–C-073 und C-074–C-077. Diese Datei ersetzt keine davon; alle zehn
> bleiben nebeneinander erhalten. Die englische Schwesterfassung
> `docs/Evolution Architecture V1 to V2.md` wird weiterhin nicht nachgeführt.
>
> Neu in dieser Fassung sind ausschließlich die Deltas C-078–C-079. Beide
> machen eine Regel ausführbar, die bislang eine Ermessensentscheidung des
> Implementierers offenließ. Es handelt sich um
...
> C-001–C-077 bleibt als historische Basis erhalten und wird nicht umgedeutet;
...
>
> Nächste freie ID nach dieser Runde: C-080.
...
| C-078 | Messproblem | Die Grenze zwischen Prüfstufe 2 und 4 wird vor dem Aufbau der Warteschlange als versioniertes Strukturkriterium aus einem eingefrorenen Graph-Snapshot festgelegt und im Manifest gespeichert; sie gilt für historieunbekannte Memories außerhalb der Stufen 1 und 3, die Auswertung läuft 1 → 3 → 2 → 4, und Graphausfall führt konservativ zu Stufe 4. |
| C-079 | Architekturentscheidung | Der semantische Vorschlagshash wird aus einer versionierten kanonischen Struktur gebildet und enthält keinen freien Erklärungstext; die Quellversion bezeichnet eine semantische Inhaltsversion, nicht `updated`. |
...
**Finaler Delta-Fix, 26.07.2026 (diese Fassung):** Zwei Punkte, die beide
dieselbe Schwäche beheben: Eine Regel war formuliert, aber nicht ausführbar,
und die Lücke lag jeweils im Ermessen des Implementierers. C-078 legt die
Grenze zwischen Prüfstufe 2 und 4 vorab und versioniert fest, damit sie nicht
während des Laufs wandert. C-079 kanonisiert den semantischen Vorschlagshash,
damit eine bloße Umformulierung ihn nicht bewegt.

**Nächste freie ID: C-080.** Neue Delta-Reviews beginnen dort. Ein Urteil
...
ein importiertes Memory mit Brückenstellung bleibt deshalb Stufe 3 und wandert
nicht über das Strukturkriterium nach Stufe 2.
...
Verlinkungsgrad oder eine Brückenstellung zwischen sonst unverbundenen
Bereichen. Wer sie nachweislich besitzt, fällt in Stufe 2; wer sie nicht
besitzt, in Stufe 4.
...
- die **Brückeneigenschaft** eines Knotens — er verbindet mindestens zwei sonst
  getrennte Cluster; diese Eigenschaft wird von der Graph-Projektion bereits
  berechnet;
...
## 28. Delta-Ledger (C-029–C-079)

Dieser Abschnitt dokumentiert neun aufeinanderfolgende Runden von Deltas
...
**Runde 9 — C-078 und C-079** ist der finale Delta-Fix. Beide Einträge machen
eine Regel ausführbar, die eine Ermessenslücke offenließ:
...
- **Ausführbar gemacht durch C-078.** „Nachweislich hohe Strukturwirkung“ war
  hier nicht definiert und lag damit im Ermessen der Implementierung.
  Verbindlich ist das vorab versionierte Kriterium aus 6.3.
...
- **Evidenz:** C-076 verlegte die Grenze zwischen Prüfstufe 2 und 4 auf
  ...
  einen Knoten als Brücke, sobald er mindestens zwei sonst getrennte Cluster
  verbindet.
...
- **Abnahmekriterium:** Das Kriterium liegt vor dem Aufbau der Warteschlange
  ...
  ausschließlich aus Brückeneigenschaft und Grad-Schwellenwert oder
  Grad-Quantil. Der numerische Wert steht vor der Zuordnung fest und wandert
...
## 32. Übergabe nach dem finalen Delta-Fix

**Was geändert wurde.** Diese Fassung fügt die Deltas C-078 und C-079 hinzu.
Geändert wurden ausschließlich die folgenden Zeilenbereiche:

| Passage | Zeilen | Delta |
|---|---|---|
| Titel und Präambel | 1–33 | alle |
| 0.4 Korrekturverweise an C-073, C-075, C-076 | 175–178 | C-078, C-079 |
| 0.4 Ledgerzeilen C-078 und C-079 | 180–181 | alle |
| 0.4 Abnahmeblock und nächste freie ID | 276–285 | alle |
| 6.3 Mapping-Stufen eindeutig benannt | 876–882 | redaktionell |
| 6.3 Prüfstufentabelle und Auswertungsreihenfolge | 1115–1131 | C-078 |
| 6.3 Strukturkriterium vorab festgelegt | 1150–1194 | C-078 |
| 14.4 Fingerprint-Versionsbegriff | 2365–2366 | C-079 |
| 14.4 kanonische Hashstruktur und Abgrenzung | 2390–2429 | C-079 |
| 14.4 materielle Größe Reason-Code | 2446–2447 | C-079 |
| 14.4 Ausschlussliste | 2451–2455 | C-079 |
| 18.5 M4-Gates | 3243–3295 | C-078, C-079 |
| 24 Verbotsliste | 3776–3788 | C-078, C-079 |
| 28 Überschrift und Rundenzahl | 3926–3928 | alle |
| 28 Runde-9-Zuordnungstabelle | 4036–4042 | alle |
| 28 Korrekturverweis an C-065 | 5087–5092 | C-078 |
| 28 Rollback in C-071 nachgezogen | 5248–5252 | C-078 |
| 28 Korrekturverweise und Abnahmekriterium an C-073, C-075, C-076 | 5300–5404 | C-078, C-079 |
| 28 Delta-Blöcke C-078 und C-079 | 5432–5496 | alle |
| 32 dieser Abschnitt | ab 5772 | — |

Alle übrigen Passagen sind unangetastet. Produktcode, englische
Schwesterfassung und die neun Vorgängerfassungen wurden nicht verändert.

**Was die beiden Deltas beheben.** Beide schließen dieselbe Art von Lücke: eine
Regel war formuliert, aber ihre Anwendung lag im Ermessen der Implementierung.

1. **Das Strukturkriterium (C-078).** „Nachweislich hohe Strukturwirkung“ war
   nicht definiert. Zwei Läufe über denselben Bestand hätten verschiedene
   Warteschlangen erzeugt, und ein während des Laufs nachjustierter
   Schwellenwert hätte eine nicht rekonstruierbare Reihenfolge produziert. Das
   Kriterium wird jetzt vor dem Aufbau der Warteschlange aus einem eingefrorenen
   Graph-Snapshot bestimmt, im Manifest versioniert und wandert während des
   Laufs nicht. Graphausfall führt konservativ nach Stufe 4.
2. **Die Hashform (C-079).** C-075 benannte die Bestandteile des Hashes, nicht
   ihre Form. Ginge die Begründung als Prosa ein, hätte eine bloße
   Umformulierung eine Wiedervorlage ausgelöst — genau die Schleife, gegen die
   C-067 und C-073 angetreten sind. Ebenso hätte „Quellversion“ als `updated`
   gelesen jeden Speichervorgang zur Wiedervorlage berechtigt. Der Hash wird
   jetzt aus einer versionierten kanonischen Struktur gebildet: Reason-Code
   statt Prosa, semantische Inhaltsversion statt `updated`, sortierte Mengen und
   eine Schema-Version.

**Ist-Claims.** Die beiden in C-078 zugelassenen Bestandteile existieren
bereits: Die Graph-Projektion führt je Knoten die Kantenzahl
(`packages/core/src/graph.ts:36`) und markiert einen Knoten als Brücke, sobald
er mindestens zwei sonst getrennte Cluster verbindet (`:38`–`:39`); erzeugt wird
beides von `buildGraph` (`:226`). Es ist keine neue Graphauswertung zu bauen.
...
1. Ob der Grad-Schwellenwert absolut oder als Quantil sinnvoller ist — das
   ...
   Bestandsverteilung.
2. Ob „eingefrorener Graph-Snapshot“ eine eigene Persistenz braucht oder ob ein
   Zeitstempel auf der bestehenden Projektion genügt.
3. Ob das Reason-Code-Vokabular vollständig genug ist, um jede materielle
   Begründungsänderung abzubilden, ohne zum Freitext zu degenerieren.
...
`max_provenance_hops` = 2 als Startkandidat. Die englische Schwesterfassung
liegt neun Revisionen zurück. Nebenbefund ohne C-ID: Die Daemon-README
beschreibt abgelaufene Memories als „(or excluded if expired)"; der Code dämpft
sie nur auf 20 %.

**Nächste freie ID: C-080.**

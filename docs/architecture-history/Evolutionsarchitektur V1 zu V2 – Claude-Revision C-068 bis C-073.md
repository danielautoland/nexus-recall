# Bastra Recall – Evolutionsarchitektur V1 → V2 (Claude-Revision C-068 bis C-073)
...
> C-029–C-039, C-040–C-048, C-049–C-054, C-055–C-059, C-060–C-062 und
> C-063–C-067. Diese Datei ersetzt keine davon; alle acht bleiben nebeneinander
> erhalten. Die englische Schwesterfassung
> `docs/Evolution Architecture V1 to V2.md` wird weiterhin nicht nachgeführt.
>
> Neu in dieser Fassung sind ausschließlich die Deltas C-068–C-073. Sie
> beheben zwei Widersprüche, die mit der Umsetzung der
> Product-Owner-Entscheidungen entstanden sind, korrigieren eine Ist-Aussage
> und operationalisieren drei Regeln, die bislang nur beschreibend formuliert
> waren. Es handelt sich um
> einen reinen Delta-Fix, nicht um eine Vollrevision: Passagen ohne Delta sind
...
> C-001–C-067 bleibt als historische Basis erhalten und wird nicht umgedeutet;
...
> die Umsetzung. Produktcode und englische Fassung werden nicht verändert.
>
> Nächste freie ID nach dieser Runde: C-074.
...
| C-067 | Product-Owner-Vorgabe | Ein abgelehnter Konsolidierungsvorschlag kehrt nur zurück, wenn er inhaltlich ein anderer ist; ein geänderter Fingerprint ist notwendige, aber nicht hinreichende Bedingung. **Operationalisiert durch C-073:** Inhaltshash plus benannte materielle Änderung. |
...
| C-069 | Ist-Korrektur | Der gesamte Bestand ist review- und bestätigungsfähig. Importstatus und `write_origin` bestimmen ausschließlich die abgeleitete Ausgangsklasse und einen Vorschlagswert der Oberfläche, niemals die Reviewberechtigung. |
...
| C-071 | Ist-Korrektur | Pro-Memory-Nutzungstelemetrie existiert bereits als Usage-Sidecar mit `surfaced`, `loaded`, `acted_on` und Zeitstempeln; Prüfstufe 2 benennt sie als Quelle. Fehlende Historie bedeutet `unknown`, nicht null. |
| C-072 | Messproblem | Der Versuch zum Cue-Erzeugungsweg wird eindeutig: entweder gepaarter Agent-gegen-Batch-Vergleich bei festgehaltenen Cue-Achsen (Anlage A) oder vollständig gekreuztes 2×2×2 mit eigenem Mindest-N (Anlage B). Die Anlage wird vor dem Lauf registriert. |
| C-073 | Architekturentscheidung | Die Wiedervorlage eines abgelehnten Vorschlags verlangt geänderten Fingerprint **und** eine benannte materielle Änderung; gleicher Vorschlagsinhalt bleibt unterdrückt. |
...
**Gegenreview der Entscheidungsumsetzung, 25.07.2026 (diese Fassung):** Zwei
...
**Nächste freie ID: C-074.** Neue Delta-Reviews beginnen dort. Ein Urteil
...
Die Ableitung vergibt **keinen** Review-Status. Der Status ergibt sich
...
**Keine Stufe-2-Beobachtung erzeugt `user_asserted`.** Diese Klasse entsteht
...
die Herkunft nicht bei sich selbst verortet, und die Rückfrage lässt sich mit
einem Vorschlagswert vorbelegen (siehe unten).
...
Abschnitt darf die Auswahl als **Vorschlagswert** vorbelegen — sie darf sie nicht einschränken. Ein importiertes Memory startet
mit dem Vorschlag `imported_unverified`, ein `agent-session`-Memory mit
`unknown_legacy`; der Nutzer kann in beiden Fällen jede Antwort wählen,
einschließlich „Ja, das stammt von mir“. Die Ableitung ist eine Vermutung des
Systems, die Antwort ist die Entscheidung des Nutzers.
...
| 2 | häufig ausgespielte und häufig genutzte Memories | Ihre Herkunft wirkt sich am häufigsten aus |
| 3 | importierte und als `imported_unverified` geführte Memories | Fremde Urheberschaft, heute pauschal ungeklärt |
| 4 | der übrige Bestand | sukzessive, ohne Zeitdruck |

Innerhalb einer Stufe gilt keine feste Reihenfolge. Die Stufen sind eine
Priorisierung der Arbeit, keine Abstufung der Verbindlichkeit: Auch Stufe 4
wird vollständig geprüft.
...
Die ausführbare Konsequenz: Memories ohne Sidecar-Historie werden als **eigene,
sichtbar gekennzeichnete Teilmenge der Stufe** geführt. Sie werden nicht als
„nie genutzt" gewertet, nicht aus Stufe 2 herausgenommen und nicht in eine
spätere Stufe verschoben. Weil das Häufigkeitskriterium bei ihnen nicht
auswertbar ist, entscheidet für sie ein **historieunabhängiges Zweitkriterium**:
Floor- oder Pin-Status und Verlinkungsgrad im Graphen. Stufe 2 hat damit zwei
Kriterien — Nutzungshäufigkeit, wo eine Historie vorliegt, und
Struktur­bedeutung, wo keine vorliegt.
...
- Der Override wirkt allein auf `provenance_class` und überstimmt dort die
  Stufe-1- und Stufe-2-Ableitung bis zum Widerruf oder bis zur inhaltlichen
...
- Operatortyp;
- beteiligte Memory-IDs und deren Versionen;
...
Fingerprint einen **Hash des normalisierten Vorschlagsinhalts** — also der
vorgeschlagenen Zustandsänderung selbst, nicht ihres Umfelds. Eine
Wiedervorlage setzt alle drei Bedingungen voraus:

1. einen **geänderten Fingerprint** nach der obigen Liste,
2. einen **geänderten Hash des Vorschlagsinhalts**, und
...
- der **Begründung**, sofern sie sich auf neue Evidenz stützt;
- einem zuvor **gelösten Schutzkonflikt** — etwa einer weggefallenen
  Sensitivity- oder Invariantenverletzung.

Ist der Hash des Vorschlagsinhalts unverändert, bleibt der Vorschlag
unterdrückt — unabhängig davon, wie sich der Fingerprint bewegt hat. Lässt sich
...
- für den Cue-Erzeugungsweg aus 18.3 wird vor dem Lauf registriert und
  ...
  Anlage A mit getrennter Zellstruktur bei festgehaltenen Cue-Achsen oder
  Anlage B als vollständig gekreuztes 2×2×2 —, jeweils mit Zellstruktur,
  Mindest-N je Zelle und Auswertungsregel.
...
- eine registrierte Cue-Versuchsanlage (A oder B) mit Zellstruktur, Mindest-N
  je Zelle und Auswertungsregel liegt versioniert vor.
...
**Anlage A — gepaarter Vergleich bei festgehaltenen Cue-Achsen (empfohlen).**
Die Cue-Achsen werden auf einer Konfiguration festgehalten — sinnvollerweise
der aus dem 2×2 als beste hervorgegangenen —, und für dieselben Goldfälle
werden Agenten- und Batch-Cues paarweise erzeugt und verglichen. Der Vergleich
ist gepaart, weil beide Wege auf denselben Fällen laufen; das senkt das nötige
N erheblich. Empfohlen, weil er ohne Vervielfachung der Zellen auskommt und die
Frage direkt beantwortet.

**Anlage B — vollständig gekreuztes 2×2×2.** Der Erzeugungsweg tritt als dritter
Faktor neben die beiden Cue-Achsen. Das erlaubt Aussagen über Wechselwirkungen
zwischen Erzeugungsweg und Cue-Achse, verdoppelt aber die Zellenzahl auf acht
und benötigt ein **eigenes, entsprechend größeres Mindest-N je Zelle**, das nach
18.1 vor dem Lauf festzulegen ist.
...
**Konstante Umgebung.** Kaskade, Dämpfung, Reranker, Pool und Query-Klassen
bleiben über **alle Zellen der registrierten Anlage** konstant und entsprechen
dem für M2 festgelegten Referenz-Setup — vier Zellen bei Anlage A, acht bei
Anlage B. Variiert werden ausschließlich die Faktoren der registrierten
Anlage. Der Versuch wird **nicht** ungeplant mit den übrigen
...
- ein abgelehnter Vorschlag kehrt nur zurück, wenn Fingerprint **und** Hash des
  normalisierten Vorschlagsinhalts sich geändert haben und mindestens eine der
  fünf materiellen Größen aus 14.4 benannt ist; Ablehnungsprotokoll,
  Inhaltshash und Vorschlagsvergleich sind je Struktur nachweisbar;
...
- keine Behandlung fehlender Usage-Historie als Nutzung null;
...
## 28. Delta-Ledger (C-029–C-073)

Dieser Abschnitt dokumentiert sieben aufeinanderfolgende Runden von Deltas
...
**Runde 5 — C-060 bis C-062** ist der abschließende Delta-Fix. Die Zuordnung
zur Runde 4:
...
| C-067 | C-073 | Operationalisierung: Inhaltshash und materielle Änderung |
...
- **Rollback:** Die Prüfung ändert keine Memory-Inhalte und lässt sich jederzeit
  ...
  nutzbar.
- **Präzisiert durch C-071.** Die Datenquelle für Prüfstufe 2 ist der
  vorhandene Usage-Sidecar; es ist keine neue Telemetrie zu bauen, und fehlende
  Historie bedeutet `unknown`, nicht null.
...
- **Operationalisiert durch C-073.** „Inhaltlich ein anderer“ ist hier keine
  prüfbare Größe; verbindlich sind Inhaltshash und die fünf materiellen Größen
  aus 14.4.
...
- **Abnahmekriterium:** Die Oberfläche fragt bei der Auswahl „Vom Agenten oder
  ...
  Zusammenfassung zu einer Klasse. Die Rückfrage darf vorbelegt, aber nicht
  übersprungen werden.
...
- **Rollback:** Fällt die Quelle aus, greift für alle Memories das
  historieunabhängige Zweitkriterium aus 6.3 — Floor- oder Pin-Status und
  Verlinkungsgrad; die Prüfung bleibt vollständig, nur ihre Priorisierung wird
  gröber.
...
- **Gate:** M4.
- **Datenquelle:** Ablehnungsprotokoll je Struktur mit Fingerprint und Hash des
  normalisierten Vorschlagsinhalts.
- **Abnahmekriterium:** Eine Wiedervorlage setzt einen geänderten Fingerprint
  **und** einen geänderten Inhaltshash **und** mindestens eine benannte
  materielle Änderung voraus — an Zielzustand, Quellversion, Kantenmenge,
  Begründung auf Basis neuer Evidenz oder einem gelösten Schutzkonflikt. Bei unverändertem Inhaltshash
...
der gepaarte Agent-gegen-Batch-Vergleich bei festgehaltenen Cue-Achsen
(empfohlen), oder Anlage B, ein vollständig gekreuztes 2×2×2 mit eigenem,
größerem Mindest-N. Welche Anlage gefahren wird, ist vor dem Lauf zu
registrieren. Ein
...
`write_origin` liefern nur den Vorschlagswert. `not_scheduled` bezeichnet nur noch die
...
## 32. Übergabe nach dem Gegenreview der Entscheidungsumsetzung

**Was geändert wurde.** Diese Fassung behebt die Befunde des Gegenreviews als
Deltas C-068–C-073. Geändert wurden ausschließlich die folgenden
Zeilenbereiche:

| Passage | Zeilen | Delta |
|---|---|---|
| Titel und Präambel | 1–35 | alle |
| 0.4 Korrekturverweise an C-049, C-054, C-055, C-057, C-063–C-067 | 153–171 | C-068–C-073 |
| 0.4 Ledgerzeilen C-068–C-073 | 172–177 | alle |
| 0.4 Abnahmeblock und nächste freie ID | 251–263 | alle |
| 6.3 Stufe-2-Tabelle ohne Review-Status | 874–882 | C-069 |
| 6.3 progressive Oberflächenauswahl | 953–977 | C-068 |
| 6.3 Vorschlagswert statt Einschränkung | 983–988 | C-069 |
| 6.3 Ableitung bestimmt nur die Ausgangsklasse | 1014–1022 | C-069 |
| 6.3 Startwert `pending` unabhängig von `write_origin` | 1047–1048 | C-069 |
| 6.3 Datenquelle für Stufe 2 und Zweitkriterium | 1084–1103 | C-071 |
| 6.3 Bindung an Memory-ID und Inhaltshash | 1116–1145 | C-070 |
| 7.2 Begriffsklärung Review | 1251–1252 | C-068 |
| 14.4 Wiedervorlage mit Inhaltshash | 2278–2301 | C-073 |
| 18.1 Registrierung der Versuchsanlage | 2645–2649 | C-072 |
| 18.1 M0-Gate für die registrierte Anlage | 2659–2660 | C-072 |
| 18.3 zwei zulässige Versuchsanlagen | 2825–2847 | C-072 |
| 18.3 Rückverweis auf Variante (a)/(b) | 2849–2850 | C-072 |
| 18.3 konstante Umgebung je Anlage | 2867–2871 | C-072 |
| 18.5 M4-Gates | 3047–3086 | C-068–C-073 |
| 24 Verbotsliste | 3556–3566 | C-068–C-072 |
| 26.2 Promotionskriterien | 3669–3671 | C-068 |
| 28 Überschrift und Rundenzahl | 3704–3706 | alle |
| 28 Runde-7-Zuordnungstabelle | 3792–3802 | alle |
| 28 Korrekturverweise an C-049, C-055, C-063–C-067 | 4326–4896 | C-068–C-073 |
| 28 Delta-Blöcke C-068–C-073 | 4902–5042 | alle |
| 31 Entscheidung 1 Auflage | 5233–5237 | C-072 |
| 31 Entscheidung 5 Auswirkung | 5300–5303 | C-068, C-069 |
| 32 dieser Abschnitt | ab 5316 | — |

Alle übrigen Passagen sind unangetastet. Produktcode, englische
Schwesterfassung und die sieben Vorgängerfassungen wurden nicht verändert.

**Die beiden Blocker.** Beide entstanden bei der Umsetzung von C-063 und
betreffen dieselbe Passage:

1. **Drei Herkünfte auf eine Klasse (C-068).** Die Oberflächenzuordnung bildete
   „beobachtet oder abgeleitet“ vollständig auf `agent_observed` ab, obwohl die
   Klassentabelle `agent_observed`, `derived` und `hypothesis` als drei
   epistemische Zustände führt. Die Zusammenfassung hätte die Unterscheidung
   ausgerechnet dort eingeebnet, wo sie zum ersten Mal jemand mit Wissen treffen
   kann — `derived` und `hypothesis` wären dauerhaft leer geblieben. Die
   Oberfläche fragt jetzt progressiv nach.
2. **Reviewberechtigung an `write_origin` gekoppelt (C-069).** Nach der
   Einarbeitung blieb der Satz stehen, ein nicht importierter Save mit
   `write_origin: user-directed` sei „die einzige Konstellation, die überhaupt
   für eine spätere Bestätigung in Betracht kommt“. Das widerspricht C-065
   unmittelbar und hätte den größten Teil des Bestands faktisch von der
   Bestätigung ausgeschlossen. Ableitung und Importstatus liefern jetzt
   ausschließlich Ausgangsklasse und Vorschlagswert.

**Der Ist-Befund.** C-071 korrigiert eine Aussage, die ich selbst aufgestellt
hatte: Die Pro-Memory-Nutzungstelemetrie für Prüfstufe 2 fehlt nicht. Der
Usage-Sidecar unter `.bastra/usage/` führt je Memory-ID `surfaced`, `loaded`
und `acted_on` samt Zeitstempeln, stellt Heat- und Reichweitenberechnung bereit
und ist im Daemon angeschlossen. Neu ist nur die Regel, dass fehlende Historie
`unknown` bedeutet und nicht `0` — eine Unterscheidung, die bei einem Sidecar,
der erst ab einem bestimmten Stand mitschreibt, über die Priorisierung
entscheidet.

**Drei Operationalisierungen.** C-070 grenzt ab, was zum bestätigten Inhalt
zählt — Titel, Summary, Body und die Aussage selbst, nicht aber Tags,
`topic_path`, `recall_when`, Cues, Ordner, Zeitstempel, Heat und
Nutzungssignale. Ohne diese Grenze wäre jede Bestätigung binnen Stunden
erloschen, weil bereits ein Recall-Treffer die Nutzungssignale bewegt. C-072
ersetzt die nicht auswertbare Formel „eigener Faktor, aber kein zusätzlicher
Arm“ durch zwei zulässige Versuchsanlagen mit Registrierungspflicht. C-073
macht „inhaltlich ein anderer Vorschlag“ prüfbar: Inhaltshash plus mindestens
eine von fünf benannten materiellen Änderungen.

**Neue Ist-Claims, am HEAD belegt.**

- Der Usage-Sidecar führt je Memory-ID `surfaced`, `loaded`, `acted_on` und die
  Zeitstempel des jeweils letzten Ereignisses und stellt eine Heat- sowie eine
  Reichweitenberechnung bereit — `packages/daemon/src/usage-sidecar.ts`.
- Der Sink ist im Daemon angeschlossen; `recordUsage` wird dort importiert und
  über den `onUsage`-Hook gespeist — `packages/daemon/src/index.ts:39` und
  `:231`.

**Was besonders zu prüfen ist.**

1. Ob die progressive Rückfrage in 6.3 die drei Zustände für einen Nutzer ohne
   Systemkenntnis unterscheidbar formuliert — „direkt beobachtet“ gegenüber
   „abgeleitet“ ist für den Agenten klar, für den Menschen möglicherweise
   nicht.
2. Ob der Vorschlagswert der Oberfläche (C-069) die Antwort faktisch doch
   steuert, wenn er vorbelegt ist.
3. Ob die Hash-Abgrenzung in 6.3 vollständig ist — insbesondere, ob `type` und
   `scope` in jeder Darstellung des Reviews sichtbar sind, da sie sonst zum
   Hash zählen, ohne dass der Nutzer sie bestätigt hat.
4. Ob Anlage A in 18.3 die Frage wirklich beantwortet, wenn die Cue-Achsen auf
   der Siegerkonfiguration festgehalten werden — ein Erzeugungsweg könnte auf
   einer anderen Achse besser abschneiden.
5. Ob die fünf materiellen Größen in 14.4 vollständig sind und ob „Begründung,
   sofern sie sich auf neue Evidenz stützt“ ohne Zirkelschluss prüfbar ist.
6. Ob `unknown` gegenüber `0` beim Usage-Sidecar in der Priorisierung eine
   ausführbare Konsequenz hat oder nur eine Feststellung bleibt.
...
M0-Baseline-Run; `max_provenance_hops` = 2 als Startkandidat. Die englische
Schwesterfassung liegt sieben Revisionen zurück. Nebenbefund ohne C-ID: Die
Daemon-README beschreibt abgelaufene Memories als „(or excluded if expired)“;
der Code dämpft sie nur auf 20 %.

**Was die Änderung trägt.** Für C-071 der Code am HEAD mit den beiden genannten
Fundstellen; für die beiden Blocker der Widerspruch zwischen den betroffenen
Passagen desselben Dokuments; für die drei Operationalisierungen die bestehenden
Messgates M0 bis M6, deren Freigabelogik unverändert bleibt.

**Nächste freie ID: C-074.**

# Bastra Recall – Evolutionsarchitektur V1 → V2 (Claude-Revision C-060 bis C-062)
...
> C-029–C-039, C-040–C-048, C-049–C-054 und C-055–C-059. Diese Datei ersetzt
> keine davon; alle sechs bleiben nebeneinander erhalten. Die englische
> Schwesterfassung `docs/Evolution Architecture V1 to V2.md` wird weiterhin
> nicht nachgeführt.
>
> Neu in dieser Fassung sind ausschließlich die Deltas C-060–C-062, zuzüglich
> dreier redaktioneller Korrekturen ohne eigene C-ID. Es handelt sich um einen
> reinen Delta-Fix, nicht um eine Vollrevision: Passagen ohne Delta sind
...
> C-001–C-059 bleibt als historische Basis erhalten und wird nicht umgedeutet;
...
> werden von dieser Fassung nicht vorweggenommen. Produktcode und englische
> Fassung werden nicht verändert.
>
> Nächste freie ID nach dieser Runde: C-063.
...
| C-055 | Ist-Korrektur | **Attestierungsbegriff korrigiert durch C-060:** Ein Mutation-Audit genügt nicht, es braucht einen nicht selbst behauptbaren Bestätigungsbezug. Ein vom Caller gesetztes `write_origin: user-directed` ist keine Attestierung. `user_asserted` entsteht automatisch nur über einen serverseitig belegten vertrauenswürdigen Schreibpfad; die Review führt `confirmed_provenance_class` mit Auditbezug, und die Provenienzbestätigung ist ein eigener widerrufbarer Vertrag, kein Klasse-B-Override. |
...
| C-060 | Ist-Korrektur | Ein Mutation-Audit belegt die Mutation, nicht den Nutzerakt: Der Audit-Kontext wird vom Caller geliefert und fällt sonst auf `actor: user` zurück. Attestierung verlangt einen nicht vom speichernden Caller behauptbaren Bestätigungsbezug; ohne ihn entsteht kein automatisches `user_asserted`. |
...
| C-062 | Architekturentscheidung | Survival- und Zitationsquote gelten je Berechtigungs-, Scope- und Sensitivity-Kontext mit Zielwert 100 % innerhalb jedes Kontextes; die Wiederholungssperre beruht auf einem strukturellen Fingerprint statt auf Zeit oder Cache. |
...
**Letzter Delta-Fix, 25.07.2026 (diese Fassung):** Drei Punkte. C-060
...
**Nächste freie ID: C-063.** Neue Delta-Reviews beginnen dort. Ein Urteil
...
| Wert | Bedeutung |
|---|---|
| `user_asserted` | vom Nutzer oder einer autorisierten Quelle **belegt** — Attestierungsbezug oder bestätigte Review |
...
| Beobachtung | abgeleitete Klasse |
|---|---|
| `write_origin: user-directed` **und Attestierungsbezug** | `user_asserted` |
| `write_origin: user-directed` **ohne Attestierungsbezug** | `unknown_legacy`, Review-Status `pending` |
...
**Attestierung** heißt deshalb: Der Schreibvorgang trägt einen
Bestätigungsbezug — `user_action_ref` beziehungsweise `confirmation_ref` —, der
...
Provenienz-Review bleibt in diesem Fall der reguläre Weg zu `user_asserted` —
er ist nicht versperrt, sondern läuft nur nicht automatisch.
...
**Trust-Grenze Mac-Client.** Soll der Mac-Client als vertrauenswürdiger Attestor
gelten, ist das eine ausdrücklich zu benennende Vertrauensentscheidung und keine
Folge seines Audits. Sie setzt voraus, dass der Client eine **vom normalen
Save-Aufruf getrennte Nutzeraktionsreferenz** erzeugt und übermittelt — etwa aus
einer Bestätigungsinteraktion —, die der Save-Pfad nicht selbst herstellen kann.
Bis eine solche Referenz existiert und dokumentiert ist, ist auch der
Mac-Bridge-Pfad nicht attestiert.

Kein heute existierender Schreibpfad erfüllt diesen Vertrag. Die Rückfallregel
gilt deshalb bis auf Weiteres für **alle** Pfade; sie entfällt für einen Pfad
erst, wenn dieser den Bestätigungsbezug nachweislich erfüllt. Ein einheitliches
Mutation-Audit für MCP- und HTTP-Saves — nach Abschnitt 23 gesonderte spätere
Arbeit — verbessert die Nachvollziehbarkeit, ändert daran aber nichts.
...
freigeben. Andernfalls entstünde genau die Schleife, die diese Regel verhindert
...
- Ein nicht attestiertes `write_origin: user-directed` verliert keine
  Funktion — es führt lediglich nicht automatisch zu `user_asserted`. Da heute
  kein Schreibpfad einen Attestierungsbezug erzeugt, gilt das übergangsweise
  für alle Pfade.
...
## 28. Delta-Ledger (C-029–C-062)

Dieser Abschnitt dokumentiert fünf aufeinanderfolgende Runden von Deltas
...
- **Korrigiert durch C-049 in zwei Punkten.** Erstens ist das hier beschriebene
  ...
  Attestierung ist kein Attribut eines Schreibpfads. Der Eintrag bleibt im
  Originalwortlaut stehen; maßgeblich sind C-049, C-055, C-060 und 6.3.
...
## 32. Übergabe nach dem letzten Delta-Fix

**Was geändert wurde.** Diese Fassung ist ein reiner Delta-Fix und fügt der
Vorgängerrevision die Deltas C-060–C-062 zuzüglich dreier redaktioneller
Korrekturen hinzu. Geändert wurden ausschließlich die folgenden Zeilenbereiche:

| Passage | Zeilen | Delta |
|---|---|---|
| Titel und Präambel | 1, 11–33 | alle |
| 0.4 Korrekturverweise an C-048, C-049, C-053, C-055, C-056, C-058, C-059 | 150, 151, 155, 157–161 | C-060–C-062 |
| 0.4 Ledgerzeilen C-060–C-062 | 162–164 | alle |
| 0.4 Abnahmeblock und nächste freie ID | 217–229 | alle |
| 6.3 Klassentabelle `user_asserted` | 792 | C-060 |
| 6.3 Stufe-2-Tabelle | 840–841 | C-060 |
| 6.3 Attestierungsdefinition | 856–909 | C-060 |
| 8.4 Punkt 8 | 1176–1180 | C-061 |
| 8.5 Endbedingung 4 | 1247–1248 | C-061 |
| 8.5 Ergebnistabelle | 1276–1278 | C-061 |
| 8.5 `stop_reason`-Eingrenzung und Defektvertrag | 1291–1320 | C-061 |
| 8.5 `no_answer`-Abgrenzung gegen Teilabdeckung | 1328–1337 | C-061 |
| 8.5 Ergebnisobjekt | 1368–1385 | C-061 |
| 8.5 Oberflächenvertrag | 1396–1401 | C-061 |
| 14.4 Fingerprint-Wiederholungssperre | 2063–2084 | C-062 |
| 14.4 Quoten je Berechtigungskontext | 2094–2108 | C-062 |
| 14.4 Historical-Zugriffsvertrag | 2156–2159 | redaktionell |
| 18.4 M3-Metriken und Live-Gates | 2709–2747 | C-061 |
| 18.5 M4-Gates | 2784–2816 | C-060, C-062 |
| 21.4 Historical-Zugriff | 3131–3135 | redaktionell |
| 22 Rückwärtskompatibilität | 3186–3191 | C-060 |
| 24 Verbotsliste | 3259–3270 | C-060, C-061 |
| 26.2 Promotionskriterien | 3365–3366 | C-061 |
| 28 Runde-5-Zuordnungstabelle | 3467–3476 | alle |
| 28 Korrekturverweise an C-042, C-048, C-049, C-052, C-053, C-055, C-056, C-058 | 3788, 3946, 3998, 4092, 4127, 4193, 4232, 4289 | C-060–C-062 |
| 28 Codefundstelle `search.ts:746` in C-059 | 4305 | redaktionell |
| 28 Delta-Blöcke C-060–C-062 | 4329–4419 | alle |
| 32 dieser Abschnitt | ab 4690 | — |

Alle übrigen Passagen sind unangetastet. Produktcode, englische Schwesterfassung
und die fünf Vorgängerfassungen wurden nicht verändert.

**Was gegenüber der Vorgängerrevision sachlich falsch war.**

1. **Attestierung (C-060).** C-055 stufte den auditierten Mac-Bridge-Pfad als
   attestiert ein. Der Audit-Kontext wird dort aber vom Caller geliefert und
   fällt sonst auf `{ actor: "user" }` zurück; die einzige inhaltliche Prüfung
   greift beim Wert `assistant`. Ein Audit belegt die Mutation, nicht den
   Nutzerakt. Konsequenz: Kein heute existierender Schreibpfad ist attestiert.
2. **`controller_defect` (C-061).** C-056 führte ihn als `stop_reason` und
   machte damit einen Implementierungsfehler zu einem regulären Endzustand, der
   in die Ergebnisstatistik eingegangen wäre.
3. **`no_answer` (C-061).** Die Definition „es existiert nachweislich keine
   belastbare Evidenz“ war stärker als das, was ein erschöpfter Lauf zeigen
   kann, und verwarf partielle Evidenz stillschweigend.
4. **Globale Survival-Quote (C-062).** Sie hätte private Quellen im Nenner
   weniger privilegierter Aufrufer mitgezählt — oder zu einem
   berechtigungsüberschreitenden Shortcut verleitet, den C-058 bereits verbietet.

**Neue Ist-Claims, am HEAD belegt.**

- Der Bridge-Pfad liest `audit_context` aus den Aufrufparametern und defaultet
  auf `{ actor: "user" }` — `packages/daemon/src/bridge.ts:322` und `:325`,
  gleichlautend `:350` und `:379`.
- Die einzige inhaltliche Prüfung des Audit-Kontextes betrifft
  `actor === "assistant"` und erzwingt dort eine Begründung —
  `packages/core/src/audit-save.ts:49`.
- Der `obsolete`-Filter des Normal Recall steht auf
  `packages/core/src/search.ts:746` (zuvor fälschlich `:742` — das ist der
  Funktionskopf).

**Korrigierte Zählangabe.** Die Vorgängerfassung C-049–C-054 hat 4.017 Zeilen,
nicht 4.477. Die Fassung C-055–C-059 hat 4.511 Zeilen, also 494 mehr — die im
damaligen Übergabetext genannte Differenz war ein Zählfehler.

**Was besonders zu prüfen ist.**

1. Ob die vier Attestierungsbedingungen in 6.3 (856–909) implementierbar sind,
   ohne einen neuen vertrauenswürdigen Kanal zu erfordern — insbesondere
   Bedingung 2, die verlangt, dass der Bezug vom speichernden Caller nicht
   behauptbar ist.
2. Ob die Konsequenz tragbar ist, dass damit **kein** heutiger Pfad
   `user_asserted` automatisch erzeugt und der gesamte Bestand über die
   Provenienz-Review läuft.
3. Ob `DeepRecallDefect` (1293–1318) an allen vier Oberflächen als Fehler und
   nicht als Ergebnis ankommt, insbesondere bei MCP, wo Fehler und Ergebnisse
   denselben Kanal teilen.
4. Ob `coverage` und `unresolved_subquestions` bei `answer_found` und den
   inconclusive-Werten ebenfalls sinnvoll belegt sind oder nur bei `no_answer`.
5. Ob die Kontextdefinition in 14.4 (2090–2104) mit den bestehenden
   Scope- und Sensitivity-Achsen deckungsgleich ist oder eine dritte Achse
   einführt.
6. Ob der Fingerprint in 14.4 (2059–2080) vollständig ist — fehlt eine Größe,
   bleibt ein Vorschlag dauerhaft blockiert; ist eine zu viel drin, kehrt die
   Schleife zurück.

**Noch offene Punkte.**

- Die vier Product-Owner-Entscheidungen in Abschnitt 31 sind unverändert offen.
- Ein Bestätigungsbezug im Sinne von C-060 existiert in keinem Schreibpfad; ob
  und wo einer entsteht, ist eine Produktentscheidung.
- Power und Mindest-N je Zelle (C-057) entstehen nach dem M0-Baseline-Run.
- `max_provenance_hops` = 2 bleibt Startkandidat.
- Die englische Schwesterfassung liegt fünf Revisionen zurück.
- Nebenbefund ohne C-ID: Die Daemon-README beschreibt abgelaufene Memories als
  „(or excluded if expired)“; der Code dämpft sie nur auf 20 %.

**Was die Änderung trägt.** Für die Ist-Korrekturen der Code am HEAD mit den
oben genannten Fundstellen; für die Vertragsänderungen die bestehenden Messgates
M0 bis M6, deren Freigabelogik unverändert bleibt; für die
Konsistenzbereinigungen das Dokument selbst.

**Nächste freie ID: C-063.**

# Bastra Recall – Evolutionsarchitektur V1 → V2 (Claude-Revision C-055 bis C-059)
...
> abgenommenen Stand C-001–C-028, fortgeschrieben durch die Revisionen
> C-029–C-039, C-040–C-048 und C-049–C-054. Diese Datei ersetzt keine davon;
> alle fünf bleiben nebeneinander erhalten. Die englische Schwesterfassung
> `docs/Evolution Architecture V1 to V2.md` wird weiterhin nicht nachgeführt.
>
> Neu in dieser Fassung sind ausschließlich die Deltas C-055–C-059 aus der
> abschließenden Korrekturrunde, zuzüglich zweier redaktioneller Klarstellungen
> ohne eigene C-ID. Es handelt sich um einen reinen Delta-Fix, nicht um eine
> Vollrevision: Passagen ohne Delta sind unverändert. Jede geänderte Passage
> ist über das Ledger in 0.4 und das Delta-Ledger in Abschnitt 28 auf genau
> eine C-ID zurückführbar. Der Stand C-001–C-054 bleibt als historische Basis
> erhalten und wird nicht umgedeutet; wo ein neuer Eintrag einen älteren
> korrigiert, steht das als Korrekturverweis am älteren Eintrag.
>
> Die vier Product-Owner-Entscheidungen in Abschnitt 31 bleiben offen und
> werden von dieser Fassung nicht vorweggenommen. Produktcode wird nicht
> verändert.
>
> Nächste freie ID nach dieser Runde: C-060.
...
| C-055 | Ist-Korrektur | Ein vom Caller gesetztes `write_origin: user-directed` ist keine Attestierung. `user_asserted` entsteht automatisch nur über einen serverseitig belegten vertrauenswürdigen Schreibpfad; die Review führt `confirmed_provenance_class` mit Auditbezug, und die Provenienzbestätigung ist ein eigener widerrufbarer Vertrag, kein Klasse-B-Override. |
| C-056 | Architekturentscheidung | Der Deep-Recall-Endzustandsvertrag wird vollständig: `answer_found` erhält eine eigene Endbedingung, Fundpriorität vor Grenze, `inconclusive_budget_exhausted` nur bei echter Ressourcengrenze, sonst `inconclusive_interrupted` mit `stop_reason`; Fehler bleiben Fehler. |
| C-057 | Messproblem | M0 legt Power und Mindest-N je Zelle des 2×2-Cue-Versuchs versioniert fest und stellt getrennte beschreibende und assoziative Goldfälle bereit; bei zu kleinem N bleibt die Interaktion explorativ und trägt keine Live-Freigabe. |
| C-058 | Architekturentscheidung | Provenienz-Shortcut und Sichtbarhalten eines Zwischenknotens sind Teil desselben atomaren Klasse-A-Vorschlags; für den nicht erfüllbaren Fall gibt es einen definierten, schleifenfreien Ausweg, und Shortcuts wahren Sensitivity-Grenzen. |
| C-059 | Architekturentscheidung | `SUPERSEDE` arbeitet über Claim- und Versionsstatus und entspricht nicht dem heutigen `archive_memory`; historische Vorgänger bleiben über einen benannten Historical-/Deep-Recall-Index erreichbar, und Rollback stellt auch Speicherort und Indexierbarkeit wieder her. |
...
**Abschließende Korrekturrunde, 25.07.2026 (diese Fassung):** Fünf Punkte,
...
**Nächste freie ID: C-060.** Neue Delta-Reviews beginnen dort. Ein Urteil
...
| Wert | Bedeutung |
|---|---|
| `user_asserted` | vom Nutzer oder einer autorisierten Quelle **belegt** — attestierter Schreibpfad oder bestätigte Review |
...
| Beobachtung | abgeleitete Klasse |
|---|---|
| `write_origin: user-directed` **und attestierter Schreibpfad** | `user_asserted` |
| `write_origin: user-directed` **ohne Attestierung** | `unknown_legacy`, Review-Status `pending` |
...
**Attestierung** heißt: Der Schreibvorgang ist serverseitig einem bestätigten
Nutzerakt zuzuordnen. Als attestiert gilt ein Save nur, wenn beide Bedingungen
erfüllt sind:

1. Er lief über einen **vertrauenswürdigen Schreibpfad** — einen Pfad, der den
   Nutzerakt selbst bezeugt und nicht nur ein Feld entgegennimmt. Dazu zählen
   der auditierte Mac-Bridge-Pfad und jeder künftige Pfad mit vollständigem
   Mutation-Audit; nicht dazu zählen der reguläre MCP- und der HTTP-Save,
   solange sie kein Audit schreiben.
2. Der Vorgang hinterlässt einen **auflösbaren Auditbezug** — Pfad, Zeitpunkt
   und Vorgangskennung —, über den die Attestierung später nachprüfbar bleibt.

Ohne beides fällt der Save konservativ auf `unknown_legacy` mit Reviewbedarf
zurück. Das ist kein Misstrauen gegenüber dem Agenten, sondern die Trennung
zwischen *behauptet* und *belegt*: Nur die belegte Nutzerurheberschaft trägt die
Schutzwirkung, die `user_asserted` verspricht.

Sobald ein einheitliches Mutation-Audit für MCP- und HTTP-Saves existiert — nach
Abschnitt 23 gesonderte spätere Arbeit —, werden diese Pfade attestiert und die
Rückfallregel greift dort nicht mehr. Bis dahin bleibt sie in Kraft.
...
`pending` ist der Startwert für jedes `imported_unverified`, für
`capture-review` und für jedes nicht attestierte `user-directed`.

`not_scheduled` bedeutet ausschließlich **„nicht automatisch zur Review
eingeplant"** — es ist keine Bestätigung und kein Qualitätsurteil. Ein Memory mit
`provenance_class: unknown_legacy` und `provenance_review: not_scheduled` hat
weiterhin unbekannte Herkunft; es steht lediglich keine Prüfung an. Der frühere
Name `not_required` war irreführend, weil er nahelegte, die Frage sei geklärt.
Der Wert gilt für `agent-session` und für fehlendes `write_origin`, also für die
Fälle, in denen eine Einzelfallprüfung ohne neue Information nichts ergäbe.
...
Stufe-1-Signale in `source`, `topic_path` und Tags unverändert bleiben. Sie ist
...
- Der Override wirkt allein auf `provenance_class` und überstimmt dort die
  Stufe-1- und Stufe-2-Ableitung dauerhaft.
...
8. liefert einen der vier Ausgänge aus 8.5: `no_answer` nur bei
   deterministischer Erschöpfung, sonst `inconclusive_budget_exhausted`
   beziehungsweise `inconclusive_interrupted`.
...
4. der Lauf endete ohne erreichte Grenze — von außen beendet durch
   Nutzerabbruch, Shutdown oder Widerruf der Freigabe, oder durch einen
   Controllerdefekt abgebrochen.
...
| `no_answer` | 1 | die Suche war **deterministisch ausgeschöpft** und es existiert nachweislich keine belastbare Evidenz |
...
| `inconclusive_interrupted` | 4 | der Lauf endete ohne erreichte Grenze — von außen beendet oder durch Defekt abgebrochen; über das Vorhandensein von Evidenz ist nichts ausgesagt |
...
- `inconclusive_interrupted` gilt für jeden Halt ohne erreichte Grenze. Das
  strukturierte Feld `stop_reason` benennt die Ursache: `user_cancelled`,
  `shutdown`, `permission_revoked` oder `controller_defect`.

`controller_defect` ist der einzige Wert, der zugleich ein Fehlersignal ist: Ein
Lauf, der stehen bleibt, während noch Äste offen sind und keine Grenze gegriffen
hat, hätte weitersuchen müssen. Er wird als Ergebnis mit diesem `stop_reason`
zurückgegeben **und** als Defekt protokolliert — beides, weil der Aufrufer eine
ehrliche Antwort braucht und die Telemetrie den Fehler sehen muss.
...
interface DeepRecallResult {
   ...
   *  4 = ohne erreichte Grenze beendet (außen oder Defekt). */
  ...
  stop_reason?: "user_cancelled" | "shutdown" | "permission_revoked" | "controller_defect";
...
- ein Transport- oder interner Fehler ist keiner der vier Werte, sondern ein
  Fehler der jeweiligen Schnittstelle;
...
automatisch wiederholt. Der Curator merkt sich die Ablehnung je Struktur und
Operatortyp und schlägt dieselbe Operation erst wieder vor, wenn sich die
zugrunde liegenden Memories oder `max_provenance_hops` geändert haben. Ein
Konsolidierungslauf, der an einer Struktur blockiert, überspringt sie und läuft
weiter; er wiederholt sie nicht innerhalb desselben Laufs und blockiert nicht
die übrigen Vorschläge.
...
Deep-Recall-Index**, der ihn nach ID, Version und Zitat auflöst. Dieser Index
ist die technische Entsprechung der Abrufrolle Historical aus 6.5.
...
- Das nach M0 versionierte Mindest-N je Zelle ist Voraussetzung für jede
  Aussage über Hauptwirkungen und Interaktion. Für die Interaktion liegt es
...
- Reicht das erreichbare N für die Interaktion nicht, wird sie im Report
  ...
  erreicht ist.
- Reicht das N auch für die Hauptwirkungen nicht, ist der Arm nicht
  auswertbar und wird als solcher berichtet — nicht als Null-Befund.

**Konstante Umgebung.** Der Cue-Versuch variiert ausschließlich die beiden
Cue-Achsen. Kaskade, Dämpfung, Reranker, Pool und Query-Klassen bleiben über
alle vier Zellen konstant und entsprechen dem für M2 festgelegten
Referenz-Setup. Der Versuch wird **nicht** ungeplant mit den übrigen
...
- `inconclusive_budget_exhausted` tritt ausschließlich mit tatsächlich
  erreichter Grenze und gesetztem `limit` auf; jeder andere Abbruch meldet
  `inconclusive_interrupted` mit `stop_reason`;
...
- `SUPERSEDE` verschiebt den Vorgänger weder in den Trash noch aus dem Index;
  er bleibt über den Historical-/Deep-Recall-Index nach ID, Version und Zitat
  auflösbar;
...
- ein Historical-/Deep-Recall-Index, der superseded Vorgänger nach ID, Version
  und Zitat auflöst, bevor `SUPERSEDE` live geht. Ohne diesen Index gäbe es
...
- Bestandsmemories ohne `provenance_class` gelten als `unknown_legacy`,
  ...
  umgeschrieben.
- Ein nicht attestiertes `write_origin: user-directed` verliert keine
  Funktion — es führt lediglich nicht automatisch zu `user_asserted`.
...
- Ereignis-, Gültigkeits-, Wissens- und Ableitungszeit getrennt beantwortbar
  sind;
- Deep Recall eine erschöpfte Suche, einen Budgetabbruch und eine Unterbrechung
  von außen unterscheidet;
- eine als Nutzeraussage geführte Memory einen belegten und nicht nur
  behaupteten Ursprung besitzt;
...
## 28. Delta-Ledger (C-029–C-059)

Dieser Abschnitt dokumentiert vier aufeinanderfolgende Runden von Deltas
...
- **Korrigiert durch C-049 in zwei Punkten.** Erstens ist das hier beschriebene
  ...
  C-055: Ohne attestierten Schreibpfad entsteht auch bei nicht importierten
  Memories kein `user_asserted`. Der Eintrag bleibt im Originalwortlaut stehen;
  maßgeblich sind C-049, C-055 und 6.3.
...
- **Rollback:** Additive Sidecar-Felder. Ohne sie verhält sich das System wie
  ...
  Verweigerung einer unbelegten Einstufung.
- **Ergänzt und eingeschränkt durch C-055.** Dieser Eintrag sichert die
  Importherkunft ab, behandelt ein nicht importiertes
  `write_origin: user-directed` aber weiterhin als hinreichend. Verbindlich ist
  die Fassung in 6.3: Ohne attestierten Schreibpfad entsteht kein
  `user_asserted`. Der Review-Status heißt jetzt `not_scheduled` statt
...
- **Vervollständigt durch C-056.** Der hier festgelegte Vertrag deckte die
  Erfolgsbedingung nicht ab und ließ nicht budgetbedingte Abbrüche über
  `limit: "other"` als Budgetproblem erscheinen. Verbindlich ist die Fassung in
  8.5 mit fünf Endbedingungen und vier Ergebniswerten.
...
- **Vervollständigt durch C-058 und C-059.** Das Verhältnis von Operation und
  Shortcut, der nicht erfüllbare Fall und die Sensitivity-Schranke sind in 14.4
  nachgetragen; die Abgrenzung von `SUPERSEDE` gegen das heutige
  Archivierungsprimitiv folgt aus C-059.
...
- **Evidenz:** Das heutige Archivierungsprimitiv tut weit mehr als eine
  ...
  Der Normal Recall filtert `obsolete` ohnehin vollständig aus
  (`packages/core/src/search.ts:742`). Ein `SUPERSEDE`, das dieses Primitiv
...
## 32. Übergabe an den abschließenden Codex-Delta-Review

**Was geändert wurde.** Diese Fassung ist ein reiner Delta-Fix und fügt der
Vorgängerrevision die Deltas C-055–C-059 zuzüglich zweier redaktioneller
Klarstellungen hinzu. Geändert wurden ausschließlich die folgenden
Zeilenbereiche:

| Passage | Zeilen | Delta |
|---|---|---|
| Titel und Präambel | 1, 11–33 | alle |
| 0.4 Ledgerzeilen C-055–C-059 | 156–160 | alle |
| 0.4 Korrekturverweise an C-047, C-049, C-051, C-052, C-053 | 148, 150, 152–154 | C-055–C-059 |
| 0.4 Abnahmeblock und nächste freie ID | 202–215 | alle |
| 6.3 stale_status-Klarstellung | 732–737 | redaktionell |
| 6.3 Klassentabelle `user_asserted` | 778 | C-055 |
| 6.3 Stufe-2-Tabelle und Attestierung | 826–861 | C-055 |
| 6.3 Review-Vertrag und Provenienz-Override | 884–936 | C-055 |
| 8.4 Punkt 8 | 1128–1130 | C-056 |
| 8.5 Endbedingungen und Prioritätsregel | 1189–1220 | C-056 |
| 8.5 Ergebnistabelle, `no_answer`-Abgrenzung, Fehlerregel | 1222–1263 | C-056 |
| 8.5 Ergebnisobjekt | 1279–1297 | C-056 |
| 8.5 Oberflächenvertrag | 1304–1312 | C-056 |
| 14.4 Erhaltungsregel, Ausweg, Schleifenschutz | 1932–1977 | C-058 |
| 14.4 Sonderfall `SUPERSEDE` | 2011–2053 | C-059 |
| 18.1 M0 Power, Mindest-N, getrennte Goldfälle | 2288–2293 | C-057 |
| 18.3 Statistische Ausführbarkeit, konstante Umgebung | 2471–2493 | C-057 |
| 18.4 M3-Gates | 2593–2611 | C-056 |
| 18.5 M4-Gates | 2649–2662 | C-058, C-059 |
| 18.6.1 entschiedener M5-Prüfpunkt | 2711–2717 | redaktionell |
| 19 beschreibende Fallmenge | 2826–2829 | C-057 |
| 21.4 Historical-Index als Voraussetzung | 2983–2986 | C-059 |
| 22 Rückwärtskompatibilität | 3037–3041 | C-055, C-059 |
| 24 Verbotsliste | 3104–3112 | C-055–C-059 |
| 26.2 Promotionskriterien | 3207–3210 | C-055, C-056 |
| 28 Runde-4-Zuordnungstabelle | 3295–3307 | alle |
| 28 Korrekturverweise an C-042 und C-047 | verteilt | C-055, C-056 |
| 28 Delta-Blöcke C-055–C-059 | 3978–4127 | alle |
| 32 dieser Abschnitt | ab 4398 | — |

Alle übrigen Passagen sind unangetastet. Produktcode wurde nicht verändert, die
englische Schwesterfassung nicht nachgeführt.

**Was gegenüber der Vorgängerrevision sachlich falsch oder unvollständig war.**

1. **Attestierung (C-055).** Die Regel „nur ein nicht importierter, belegbar
   nutzergesteuerter Save wird `user_asserted`“ war nicht durchsetzbar, weil
   `write_origin` ein frei setzbares Eingabefeld ist. Ein über MCP gesetztes
   `user-directed` ist eine Behauptung des Aufrufers, kein Nachweis.
2. **`limit: "other"` (C-056).** Der in der Vorrunde eingeführte Auffangwert
   hätte jeden Nutzerabbruch und jeden Shutdown als Budgetproblem ausgewiesen —
   dieselbe Verwechslung, die C-052 eine Ebene höher verhindern sollte.
3. **`SUPERSEDE` und Archivierung (C-059).** Das Dokument grenzte `SUPERSEDE`
   gegen Klasse-B-Accessibility ab, nicht aber gegen das vorhandene
   Archivierungsprimitiv — obwohl dieses den Vorgänger aus dem Index entfernt
   und in den Trash verschiebt.

**Neue Ist-Claims, alle am HEAD belegt.**

- `write_origin` ist im öffentlichen MCP-Tool-Schema exponiert
  (`packages/daemon/src/tool-handlers.ts:1338`), und der Save-Handler reicht die
  Eingabe ohne Audit-Wrapper an `saveMemory` durch (`:857`).
- `archiveMemoryHandler` verschiebt die Datei per `moveToTrash` in den
  Vault-Trash (`packages/daemon/src/tool-handlers.ts:928`), entfernt sie per
  `forgetFile` aus dem lebenden Index (`:929`) und stempelt die Trash-Kopie
  best-effort mit `obsolete: true` und `superseded_by` (`:934`).

Die Ist-Claims der Vorrunden bleiben unverändert gültig.

**Was besonders zu prüfen ist.**

1. Ob die Attestierungsdefinition in 6.3 (826–861) die vertrauenswürdigen
   Schreibpfade hinreichend scharf fasst. Sie benennt heute den auditierten
   Mac-Bridge-Pfad und knüpft alles Weitere an das noch nicht geplante
   einheitliche Mutation-Audit aus 23.
2. Ob der Provenienz-Override-Vertrag (884–936) vollständig von Klasse B
   getrennt ist, insbesondere beim Widerruf.
3. Ob `controller_defect` in 8.5 richtig eingeordnet ist: Es ist zugleich
   Ergebniswert und Fehlersignal — bewusst, aber ungewöhnlich.
4. Ob die Prioritätsregel „Fund gewinnt gegen gleichzeitig erreichte Grenze“
   mit dem Validierungsbegriff aus Abschnitt 10 zusammenpasst.
5. Ob die Ausweichkaskade in 14.4 (1918–1963) wirklich schleifenfrei ist, wenn
   sich die zugrunde liegenden Memories zwischen zwei Läufen geringfügig
   ändern.
6. Ob der Historical-/Deep-Recall-Index in 14.4 und 21.4 ein eigener Index sein
   muss oder eine Sicht auf den bestehenden sein darf — das Dokument lässt das
   bewusst offen.
7. Ob die Sensitivity-Schranke für Shortcuts (14.4, Punkt 6) mit der
   Erreichbarkeitsgarantie kollidiert: Sie hat Vorrang, wodurch die Garantie in
   seltenen Fällen nicht erfüllbar ist. Das ist gewollt, sollte aber bestätigt
   werden.

**Noch offene Punkte.**

- Die vier Product-Owner-Entscheidungen in Abschnitt 31 sind unverändert offen.
  Keine Passage dieses Dokuments setzt eine ihrer Optionen voraus.
- Die numerischen Werte für Power und Mindest-N je Zelle (C-057) entstehen erst
  nach dem M0-Baseline-Run.
- `max_provenance_hops` = 2 bleibt Startkandidat und wird durch die Messung
  bestätigt oder korrigiert.
- Die Liste der attestierten Schreibpfade wächst mit dem einheitlichen
  Mutation-Audit; bis dahin ist nur der Mac-Bridge-Pfad attestiert.
- Die englische Schwesterfassung ist gegenüber dem Original von vier Revisionen
  im Rückstand und wird erst nach der fachlichen Abnahme der deutschen Fassung
  nachgeführt.
- Nebenbefund ohne C-ID, unverändert offen: Die Daemon-README beschreibt
  abgelaufene Memories als „(or excluded if expired)“. Der Code schließt sie
  nicht aus, sondern dämpft sie auf 20 %.

**Was die Änderung trägt.** Für die beiden Ist-Korrekturen der Code am HEAD mit
den oben genannten fünf Fundstellen; für die Vertragsänderungen die bestehenden
Messgates M0 bis M6, deren Freigabelogik unverändert bleibt; für die
Konsistenzbereinigungen das Dokument selbst.

**Nächste freie ID: C-060.**
